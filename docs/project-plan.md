...
| 3.51    | 2026-06-12 | **Strategy snapshot on the home dashboard**: `GET /api/scorecards/dashboard` (`ScorecardDashboardDto`/`AtRiskObjectiveDto`) — picks the most recent Active scorecard, resolves its measures to a RAG rollup via `IMeasureValueResolver`, and returns up to five off-track objectives (Red first, then Amber) with their worst-of rollup; `ApiClient.GetScorecardDashboardAsync`; Home.razor gains a Strategy card (gated by `Flags.ShowStrategyTools`, loaded best-effort after the main KPI fan-out) showing scorecard name/code, RAG badges, a four-segment proportional progress bar, an "Open scorecard" link, and an at-risk-objectives list with perspective tags; 3 new tests run in **fresh isolated tenants** (no active scorecard → empty, active demo → all-Green rollup, manual Red reading → surfaced as at-risk) — this isolation also removed the order-dependence the earlier dashboard draft introduced; Phase 50 suite 49 green; browser-verified on the live dashboard (card shows BSC-DEMO with 7 Green/1 Amber and the Learning & Growth accountability objective flagged for attention) |
| 3.50    | 2026-06-11 | **Phase 50 complete — final polish slice, browser-verified**: PromptMetric measures now configurable from the UI (`GET /api/scorecards/prompt-metrics` lists NumericEntry prompts with step-template name/units, `PromptMetricOptionDto`, picker in the measure modal with required-entity validation); perspective management on the detail page (move left/right with full sort-order renumber, rename/describe via pm-modal, delete with objective-count confirm modal); trend sparklines (`MeasureResponseDto.Trend` — last 12 snapshots chronological — rendered as inline 48×14 SVG polylines beside measure names); **Management Review integration (clause 9.3)** — `ManagementReview.ScorecardSummary` field + `Phase50d_ManagementReviewScorecardSummary` migration, auto-populated at review start by `BuildScorecardSummaryAsync` (active scorecard codes, measure RAG counts, Red objectives by name via `IMeasureValueResolver`), displayed in the Live Quality Snapshot panel on ManagementReviewDetail with a link to scorecards; 5 new tests (prompt-metric options list the seeded demo prompt, PromptMetric measure creates and resolves NoData without responses, trend chronology, perspective rename/reorder round-trip, review start populates scorecard summary) — Phase 50 suite 46 green; browser-verified: 3 sparklines on the demo's manual measures, 4× perspective move/rename/delete controls, Financial↔Customer reorder round-trip, PromptMetric picker populated from live data (incl. a units-duplication display fix). Remaining Phase 50 scope: none — future enhancements tracked under Phase 24+ integrations |
| 3.49    | 2026-06-11 | **Phase 50c completion + 50d integrations, browser-verified**: `ShowStrategyTools` feature flag end-to-end (`TenantFeatureFlags` field + `Phase50c_ShowStrategyToolsFlag` migration, DTO with default-true for legacy clients, signup default off, plan sync enableAdvanced‖Starter, `FeatureFlagService`, Settings → Modules toggle, NavMenu Strategy section gated); measure modal now offers all 10 source types with contextual pickers (SPC chart, training process via ProcessRole=Training, equipment for OEE) — PromptMetric remains API-only; **initiatives UI** — rocket button per objective opens a pm-modal (title/description/owner picker from `api/auth/users/picker`/due date) creating an `ActionItem` with `SourceType.StrategicObjective`, objective cards show initiative counts with open/all-closed badges (loaded via `GET api/action-items?sourceType=StrategicObjective`); **MCP tools** `get_scorecard_status` (mission/vision, RAG rollup, per-perspective objective/measure tables with live-resolved values) and `list_at_risk_objectives` (Red/Amber measures across active scorecards with gap-to-target) in new `McpController.Scorecards.cs` partial, server version 4.2; **bug fixed during browser verification**: date-input DateTime (Kind=Local) crashed Npgsql timestamptz on initiative create — converted to UTC (same latent bug exists in TeamActions.razor, flagged separately); 4 new tests (MCP status report, MCP no-Red on green demo, ShowStrategyTools flag round-trip, SpcCapability source validation) — Phase 50 suite 41 green; browser-verified: initiative launched on the demo margin objective (count badge updated), all 10 source buttons render with SPC picker appearing contextually, Strategy toggle present on Modules page |
| 3.48    | 2026-06-10 | **Phase 50c strategy map + full-capability example, browser-verified**: `ScorecardMap.razor` (/strategy/scorecards/{id}/map — SVG strategy map with perspective bands top-down Financial→Learning &amp; Growth, RAG-colored objective nodes with two-line name wrapping, cubic-curve cause-effect arrows with hover tooltips showing the causal hypothesis, Add-Cause-Link mode [click driver then outcome], edge select + Delete Link; Razor reserves `<text>` so SVG text renders via a MarkupString helper); Strategy Map button on ScorecardDetail; `DataSeeder.SeedBalancedScorecardDemoAsync` — idempotent per-tenant example exhibiting every capability: backing Kind/StepTemplate (setup+safety+numeric-prompt content → maturity 100)/Process/completed Job with 18-pass/2-fail items (90 % yield), 2 QualityCosts ($450), 5 initiatives as `ActionItem`s with `SourceType.StrategicObjective` (80 % close rate), scorecard "Demo: Widget Co. Strategy" with 4 perspectives / 7 objectives / 8 measures (live: ProcessYield, ProcessMaturity, ActionCloseRate, OpenNonConformances, QualityCost; manual: 3 with six-month snapshot trends), 2 process links, 7-edge cause chain L&amp;G→Internal→Customer→Financial; `POST /api/scorecards/seed-demo` (Admin) + Load Example button on ScorecardList; target display formatting fix (220.0000k$ → 220k$); 5 new tests (full-capability shape, all measures resolve with expected values, strategy-map chain spans all four perspectives, idempotency, initiatives created) — Phase 50 suite 37 green; verified end-to-end in the browser: login → Strategy nav → Load Example → all 8 measures resolve Green → strategy map renders bands/nodes/arrows → cause link created and deleted interactively |
| 3.47    | 2026-06-10 | **Phase 50b complete (all measure resolvers + nightly snapshots) & pm-modal restyle**: `MeasureValueResolver` gains SpcCapability (Cpk via `ISpcCalculationService` from the linked SpcChart's data points), TrainingCompliance (% current CompetencyRecords, optionally scoped to a training process), Oee (windowed `IOeeCalculationService` dashboard average, plant-wide or per equipment), QualityCost (windowed Amount sum), PromptMetric (mean numeric PromptResponse for a content block, invariant-parsed); `ValidateSource` covers SpcCapability/PromptMetric/Oee; `ScorecardSnapshotService` BackgroundService (configurable `ScorecardSnapshots:IntervalHours` default 24, per-tenant `ITenantContext.BeginScope` resolution so snapshots stamp the right TenantId, 20-hour duplicate guard, registered in Program.cs non-Testing); scorecard modals restyled to the standard `pm-modal` pattern (icon header + subtitle, `pm-input-group` fields, `pm-modal-divider` sections, pill-button source/type selectors matching the Add Kind modal) across ScorecardList create + ScorecardDetail objective/measure/snapshot/link modals; measure modal now offers QualityCost and plant-wide OEE sources with per-source hint text; 8 new tests (QualityCost sum + RAG, TrainingCompliance 100%, SPC Cpk resolution, SPC nonexistent-chart 400, OEE no-data → NoData, PromptMetric nonexistent-block 400, snapshot service captures-without-duplicates, snapshot service skips Manual) — Phase 50 suite 32 green |
| 3.46    | 2026-06-09 | **Phase 50c first slice (Strategy UI)**: NavMenu **Strategy** section (Admin/Engineer) with Balanced Scorecard entry; `ScorecardList.razor` (/strategy/scorecards — card grid with status filter/search, create modal noting auto-seeded Kaplan-Norton perspectives); `ScorecardDetail.razor` (/strategy/scorecards/{id} — mission/vision cards, RAG rollup chips, perspective columns with objective cards, measure rows with RAG dot + current/target, Activate/Archive lifecycle buttons, modals: add objective, add measure with live-source picker [Manual/ProcessYield/ProcessMaturity/WorkflowThroughput/ActionCloseRate/OpenNonConformances] and process/workflow dropdowns, record manual reading, link process/workflow); ApiClient Phase 50 section (22 methods). Remaining for 50c: strategy-map canvas view, `ShowStrategyTools` feature flag, perspective rename/reorder UI |
| 3.45    | 2026-06-09 | **Phase 50a+50b core implemented (Balanced Scorecard API)**: `Scorecard`/`ScorecardPerspective`/`StrategicObjective`/`ObjectiveMeasure`/`MeasureSnapshot`/`ObjectiveCauseLink`/`ObjectiveProcessLink` entities; `ScorecardStatus`/`StrategicObjectiveStatus`/`MeasureDirection`/`MeasureSourceType`/`MeasureCaptureSource` enums; `ActionItemSourceType.StrategicObjective`; `Phase50_BalancedScorecard` EF migration; `Phase50Dtos`; `ScorecardsController` (scorecard CRUD with four Kaplan-Norton perspectives seeded on create, perspective/objective/measure CRUD with auto codes BSC-NNN/OBJ-NNN, manual snapshots, cause-links with self/duplicate/cross-scorecard guards, process/workflow links with exactly-one-target rule, `GET /{id}/status` RAG rollup, `GET /{id}/strategy-map` nodes+edges); `IMeasureValueResolver`/`MeasureValueResolver` (Manual latest-snapshot, ProcessYield from Item grades with optional good-Grade SourceParameter, ProcessMaturity via MaturityScoringService mean, WorkflowThroughput from completed workorders, ActionCloseRate 30-day, OpenNonConformances; static `EvaluateRag` honouring HigherIsBetter/LowerIsBetter/TargetIsBest with target-attainment fallback); 24 new `Phase50ScorecardTests` (all green; full suite 1095 with 2 pre-existing flakes in QualityCostTests/UpgradeFlowTests unrelated to this change). Remaining: SpcCapability/TrainingCompliance/Oee/QualityCost/PromptMetric resolvers, `ScorecardSnapshotService` nightly capture, `ShowStrategyTools` feature flag + Strategy nav, Blazor builder/dashboard/strategy-map UI (50c), Management Review + MCP integration (50d) |
| 3.44    | 2026-06-09 | **Phase 50 design: Strategy Management — Balanced Scorecard** (Kaplan/Norton). New Strategy nav section with Balanced Scorecard builder: Scorecard/ScorecardPerspective/StrategicObjective/ObjectiveMeasure/MeasureSnapshot/ObjectiveCauseLink/ObjectiveProcessLink entities, MeasureValueResolver pulling live values from existing modules (process yield, maturity score, SPC Cpk, action close rate, OEE, training compliance, cost of quality), strategy map view, initiatives via ActionItem (`SourceType.StrategicObjective`), Management Review integration. Data model and terminology docs updated |
| 3.43    | 2026-05-05 | **Phase 34 Customer Complaint Management & VoC — complete implementation with tests, MCP tool, NavMenu + 3 new feature proposals** |
| 3.42    | 2026-05-05 | **Phase 32 Change Management & Engineering Change Order (ECO) Workflow implemented**: `ChangeOrder` entity (Code auto-generated ECO-YYYY-NNN, Type enum DesignChange/ProcessChange/DocumentChange/SupplierChange/DeviationRequest, Priority enum Routine/Urgent/Emergency, Status enum Draft/ImpactAnalysis/Approval/Implementation/Verification/Closed/Rejected, Title, Description, Justification, RequestedByUserId/DisplayName, RequestedAt, TargetImplementationDate, ClosedAt, RejectionReason); `ChangeOrderImpact` entity (AffectedEntityType enum Process/StepTemplate/ControlPlan/Pfmea/Kind/Supplier/Equipment, AffectedEntityId, AffectedEntityName, ImpactDescription, MitigationPlan); `ChangeOrderApprover` entity (UserId, DisplayName, Role, Decision enum Pending/Approved/Rejected/Abstained, DecidedAt, Comments); `ChangeOrderTask` entity (Title, Description, AssigneeUserId/DisplayName, DueDate, Status reusing ActionItemStatus, CompletedAt, CompletedByUserId, Notes); `ChangeOrderStatus`/`ChangeOrderType`/`ChangeOrderPriority`/`ApproverDecision`/`ChangeOrderImpactEntityType` enums; `ActionItemSourceType.ChangeOrder` added; `Phase32_ChangeOrders` EF migration; `Phase32Dtos` (ChangeOrderResponseDto/SummaryDto, CreateChangeOrderDto/UpdateChangeOrderDto, ChangeOrderImpactResponseDto/CreateDto, ChangeOrderApproverResponseDto/AddDto/RecordDecisionDto, ChangeOrderTaskResponseDto/CreateDto/CompleteDto, TransitionChangeOrderDto/RejectChangeOrderDto, ChangeOrderDashboardDto); `ChangeOrdersController` (Admin/Engineer, paginated list with status/type/priority/search filters, CRUD, 7-stage lifecycle transitions: Draft→ImpactAnalysis requires ≥1 impact, ImpactAnalysis→Approval requires ≥1 approver, Approval→Implementation requires all approvers decided and no rejections, Implementation→Verification requires all tasks complete, Verification→Closed; Reject from ImpactAnalysis/Approval with reason; impact CRUD; approver CRUD with duplicate guard + decision recording; task CRUD with complete endpoint; dashboard with open/closed/rejected counts + avg days to close + status/type/priority breakdowns + overdue ECOs); NavMenu Change Orders link under Conformance section; `_sectionPaths` updated; MCP `get_change_order_status` tool (status/type/priority filter, markdown summary with KPI, status/type/priority breakdowns, overdue ECO table); MCP server version 3.9 with 38 tools total; 38 new `ChangeOrderTests` (CRUD + invalid type + get by ID + not found, update fields, delete draft only, delete non-draft blocked, list paginated, filter by status, submit requires impact, submit with impact, request-approval requires approver, request-approval with approver, approve requires all decisions, approve all approved, approve with rejection fails, reject from approval, reject from draft fails, complete-implementation requires tasks done, full lifecycle draft to closed, impact CRUD + invalid entity type + list + delete, approver duplicate conflict + invalid decision + pending decision blocked + remove, task CRUD + complete + already-completed blocked + list + delete, dashboard aggregates, update closed blocked, auth required, cross-tenant isolation, MCP get_change_order_status returns data); full test suite 842 tests (841 green, 1 pre-existing flake in UpgradeFlowTests) |
| 3.41    | 2026-05-05 | **Phase 29 OEE (Overall Equipment Effectiveness) Dashboard implemented**: `ShiftDefinition` entity (Code, Name, StartTime TimeOnly, EndTime TimeOnly, IsActive); `Phase29_OeeDashboard` EF migration (ShiftDefinitions table with unique Code index); `IOeeCalculationService`/`OeeCalculationService` (per-equipment per-shift OEE calculation: **Availability** from DowntimeRecord overlap with shift window, **Performance** from StepExecution output PortTransaction counts vs ExpectedDurationMinutes ideal cycle time, **Quality** from NonConformance Scrap/Rework counts subtracted from total output, OEE = A×P×Q/10000; cross-shift night-shift handling for EndTime < StartTime; dashboard aggregation across equipment × date range × active shifts; trend time-series per equipment; Pareto loss categories from DowntimeRecord grouped by Type+Reason); `OeeController` (shifts CRUD with code uniqueness + time parsing validation, `GET /api/oee/dashboard` with date range/equipment/target filters, `GET /api/oee/trend/{equipmentId}` 7-day default, `GET /api/oee/losses` Pareto by downtime reason, `GET /api/oee/calculate` single equipment+shift+date snapshot); `Phase29Dtos` (ShiftDefinitionResponseDto, CreateShiftDefinitionDto/UpdateShiftDefinitionDto, OeeSnapshotDto with availability/performance/quality/OEE + planned/downtime/run minutes + total/good/defect piece counts, OeeTrendPointDto, OeeLossCategoryDto, OeeDashboardDto with equipment count + averages + below-target count + snapshots + top losses, OeeTrendDto); NavMenu OEE Dashboard link under Production section; `_sectionPaths` updated; MCP `get_oee_status` tool (period summary with average OEE/availability/performance/quality, equipment-level latest-shift OEE table, Pareto loss categories, optional equipment/days/target filters); MCP server version 3.8 with 37 tools total; 22 new `OeeTests` (shift CRUD + duplicate code conflict + invalid time format + get by ID + not found 404, update fields, delete, list, active-only filter, dashboard no-shifts empty, dashboard with shifts+equipment, dashboard with downtime affects availability, trend invalid equipment 404, trend valid equipment, losses list with multiple types, losses date range filter, calculate missing shift 400, calculate missing equipment 404, calculate valid parameters, auth required 401, cross-tenant shift isolation, MCP get_oee_status returns data); full test suite 804 tests (802 green, 2 pre-existing flakes in UpgradeFlowTests + WorkflowTests) |
| 3.40    | 2026-05-05 | **Phase 26 Measurement System Analysis (MSA/GR&R) implemented**: `GageStudy` entity (Name, StudyType enum GRR_Range/GRR_ANOVA/Linearity/Bias/Stability, EquipmentId FK → Equipment, ProcessId FK → Process, CharacteristicName, Tolerance, LSL, USL, NumberOfParts, NumberOfOperators, NumberOfTrials, Status enum Draft/InProgress/Complete, GrrPercent, Ndc, AcceptanceDecision); `GageStudyMeasurement` entity (GageStudyId, PartNumber, OperatorId, TrialNumber, MeasuredValue with unique composite index); `GageStudyType`/`GageStudyStatus` enums; `GrrCalculationService` (Range method: d₂/K1/K2/K3 constants for subgroup sizes 2–10, R̄ computation, EV/AV/GRR/PV/TV decomposition, %GRR of total variation, %GRR of tolerance, ndc calculation, three-tier assessment Acceptable<10%/Marginal<30%/Unacceptable); `GageStudyController` (paginated list with status/equipment filters, CRUD, bulk measurement add with bounds validation, auto Draft→InProgress on first measurement, calculate endpoint with study completion and result storage, dashboard with acceptance breakdown and worst-performing studies); `Phase26Dtos` (GageStudyResponseDto/SummaryDto, CreateGageStudyDto/UpdateGageStudyDto, GageStudyMeasurementDto, AddGageStudyMeasurementsDto/MeasurementItemDto, GrrCalculationResultDto, GageStudyDashboardDto); NavMenu Gage Studies link under Quality section; `_sectionPaths` updated; MCP `get_msa_status` tool (total/complete/draft/in-progress counts, acceptance breakdown, worst performing studies table); MCP server version 3.6 with 35 tools total; 23 new `GageStudyTests` (CRUD + invalid type + invalid equipment + get by ID + not found 404, update fields, delete, paginated list, filter by status, measurements add transitions to InProgress, invalid part/trial bounds, get measurements list, calculate with full data, calculate sets complete, insufficient data 400, tolerance percent returned, update completed blocked, add to completed blocked, dashboard aggregates, auth required, cross-tenant isolation, MCP get_msa_status returns data); full test suite 782 tests (781 green, 1 pre-existing flake in UpgradeFlowTests) |
| 3.39    | 2026-05-05 | **Phase 28 Calibration Management implemented**: `CalibrationRecord` entity (EquipmentId FK → Equipment, CalibrationType enum Internal/External/Vendor, CalibrationDate, NextDueDate, CertificateNumber, CertificateFileName, Result enum Pass/Fail/Limited, PerformedBy, StandardsUsed, TemperatureHumidity, AsFoundReading, AsLeftReading, Uncertainty, Notes); `CalibrationSchedule` entity (EquipmentId unique, IntervalDays, IntervalAdjustmentMethod enum Fixed/ReliabilityBased, ConsecutivePassCount, MaxIntervalDays, MinIntervalDays, ExtensionPercent); reliability-based interval adjustment (consecutive passes extend interval, fail resets to minimum); `CalibrationResult`/`CalibrationType`/`IntervalAdjustmentMethod` enums; `Phase28_CalibrationManagement` EF migration; `CalibrationController` (records CRUD with paginated list/equipment/result filters, equipment calibration history, schedules CRUD with active-only filter + duplicate-equipment guard + min/max validation, dashboard with due/overdue recall detection from active schedules); `Phase28Dtos` (CalibrationRecordResponseDto/SummaryDto, CreateCalibrationRecordDto/UpdateCalibrationRecordDto, CalibrationScheduleResponseDto, CreateCalibrationScheduleDto/UpdateCalibrationScheduleDto, CalibrationDashboardDto/CalibrationRecallDto); NavMenu Calibration link under Production section; MCP `get_calibration_status` tool (record stats, active schedules, overdue/due-within-30-days recall tables); MCP server version 3.5 with 34 tools; 26 new `CalibrationTests` (record CRUD + invalid equipment + invalid result + get by ID + not found, update, delete, paginated list, filter by result, equipment history, schedule CRUD + duplicate conflict + invalid equipment + min-exceeds-max, get by ID, update, delete, list, reliability-based pass extends interval, fail resets to min, fixed schedule unchanged, dashboard aggregates, overdue recalls detected, auth required, cross-tenant isolation, MCP get_calibration_status returns data); full test suite 759 tests |
| 3.38    | 2026-05-04 | **Phase 27 CAPA Workflow implemented**: `CapaRecord` entity (Code auto-generated CAPA-YYYY-NNN, Type Corrective/Preventive, SourceType 7 values, ProblemStatement, ContainmentAction, RCA link, PermanentCorrectiveAction, PreventiveAction, VerificationMethod/DueDate, VerifiedByUserId/At, EffectivenessReviewDate/VerifiedByUserId/At, Status 7-stage lifecycle, OwnerUserId/DisplayName, TeamMemberIds, ClosedAt); `CapaStep` entity (StepType, CompletedBy, Notes, AttachmentFileName); `CapaStatus` enum (Open/Containment/RootCauseAnalysis/Implementation/Verification/EffectivenessReview/Closed); `CapaType` enum (Corrective/Preventive); `CapaSourceType` enum (Manual/CustomerComplaint/InternalAudit/SupplierIssue/SpcOutOfControl/NonConformance/ManagementReview); `ActionItemSourceType.Capa` added; `Phase27_CapaWorkflow` EF migration; `Phase27Dtos` (CapaRecordResponseDto/SummaryDto, CreateCapaRecordDto/UpdateCapaRecordDto, CapaStepResponseDto/CreateCapaStepDto, TransitionCapaDto/LinkRcaDto/VerifyCapaDto, CapaDashboardDto); `CapaController` (paginated list with status/type/sourceType/search filters, CRUD, linear lifecycle transitions with gate checks: containment action required for RCA, RCA link required for Implementation, corrective action required for Verification; close requires effectiveness verified; link-rca with Ishikawa/FiveWhys validation; verify with anti-self-certification; verify-effectiveness; steps CRUD; action items CRUD; dashboard with open/overdue/closed/avgDaysToClose/byStatus/bySource/effectivenessRate/overdueCapas); `get_capa_status` MCP tool (status/type filter, markdown summary with KPI, status breakdown, source type breakdown, overdue table); NavMenu CAPA link under Quality section; `_sectionPaths` updated; MCP server version 3.4 with 33 tools total; 31 `CapaTests` (CRUD + invalid type + get by ID + not found, update fields, delete only Open, delete non-Open blocked, list paginated, filter by type, filter by sourceType, lifecycle Open→Containment, containment→RCA gate, full lifecycle, verification without corrective action fails, RCA without link fails, link-rca Ishikawa succeeds, invalid RCA type, nonexistent RCA, steps list, add step, action item create linked, action items list, dashboard aggregates, verify anti-self-certification blocks owner, verify different user succeeds, close without effectiveness fails, close after effectiveness succeeds, update closed CAPA blocked, auth required, cross-tenant isolation, MCP get_capa_status returns data) |
| 3.37    | 2026-05-04 | **Phase 25 Supplier Quality Management — tests, MCP tool, NavMenu**: 22 new `SupplierTests` (CRUD + duplicate code conflict + get by ID + not found 404, update fields, soft-delete sets Inactive, list paginated, filter by status, search by name, status transition Pending→Approved sets ApprovedDate, invalid transition Pending→Suspended blocked, Approved→Suspended valid, invalid status string 400, evaluation add with computed OverallScore, evaluation invalid scores 400, get evaluations returns list, delete evaluation, evaluations on non-existent supplier 404, dashboard aggregate with approved count, auth required 401, cross-tenant isolation 404, MCP get_supplier_quality_status returns data); `get_supplier_quality_status` MCP tool (status breakdown Approved/Conditional/Suspended/Pending, supplier table with latest evaluation score, at-risk list, optional status filter and top-N limit); NavMenu Suppliers link added under Quality section; `_sectionPaths` updated; bug fix: `SuppliersController.GetDashboard` referenced non-existent `MrbStatus.InReview` → corrected to `MrbStatus.UnderReview`; MCP server version 3.3 with 32 tools total; full test suite 702 tests (700 green, 2 pre-existing flakes in UpgradeFlowTests + WorkflowTests) |
| 3.36    | 2026-05-04 | **Phase 21 Automatic Inventory Tracking implemented**: `ApiKey` entity (SHA-256 hashed, workstation-scoped, `X-Api-Key` header auth); `Workstation` entity (Code, Name, FixedLocationId FK → StorageLocation); `ScanEvent` append-only entity (WorkstationId, ApiKeyId, ScannedBarcode, ItemId, TransactionId, Result enum, ErrorMessage, ScannedAt); `ScanResult` enum (Transferred/AlreadyAtLocation/UnknownBarcode/InvalidItemStatus/WorkstationInactive/Error); `InventoryReferenceType.Workstation` enum value; `Item.Barcode`/`StorageLocation.Barcode`/`Kind.Barcode` unique nullable fields; `Phase17_F7_BarcodeAndScanTables` EF migration (barcode columns + Workstations/ApiKeys/ScanEvents tables with indexes); `ApiKeyAuthenticationHandler` (SHA-256 hash lookup, active/expired validation, workstation_id/fixed_location_id/api_key_id claims, tenant_id claim from key, LastUsedAt update); dual authentication scheme registration (JwtBearer + ApiKey) with combined default policy; `WorkstationsController` (Admin-only: list with search/active filter + paginated, get by ID with last scan time, create with code uniqueness + location validation, update, soft-delete with active-API-key guard); `ApiKeysController` (Admin-only: list with workstationId/active filters + paginated, get by ID, create returns raw key once with SHA-256 hash storage + pk_ prefix, PATCH name/active/expiry, hard-delete); `POST /api/warehouse/scan` scan endpoint (API key auth: workstation claim extraction, workstation/location active validation, barcode resolution Item.Barcode→Item.SerialNumber fallback, item status validation Available/InProcess only, idempotent already-at-location 200 no-op, Transfer/Receipt creation based on item current location, Item.StorageLocationId update, ScanEvent audit log for all outcomes including failures, `inventory.scan` webhook event); `GET /api/warehouse/scan-events` paginated query endpoint (Admin/Engineer, filter by workstationId/result/barcode/dateFrom/dateTo); `Phase21Dtos` (WorkstationResponseDto/SummaryDto, CreateWorkstationDto/UpdateWorkstationDto, ApiKeyResponseDto/CreatedDto, CreateApiKeyDto/UpdateApiKeyDto, ScanRequestDto/ScanResponseDto/ScanItemDto/ScanLocationDto/ScanWorkstationDto, ScanEventResponseDto); ApiClient 10 Phase 21 methods (GetWorkstationsAsync, GetWorkstationAsync, CreateWorkstationAsync, UpdateWorkstationAsync, DeleteWorkstationAsync, GetApiKeysAsync, CreateApiKeyAsync, UpdateApiKeyAsync, DeleteApiKeyAsync, GetScanEventsAsync); MCP `get_workstation_status` tool (active workstations with location, API key count, last scan time, optional code/active filter); MCP server version 3.2 with 31 tools total; 24 new `Phase21Tests` (workstation CRUD + duplicate code + delete blocked with active keys + list paginated, API key create returns raw key + list filter + update + hard-delete, API key auth valid/invalid/inactive, scan transfer/unknown-barcode-404/already-at-location-idempotent/consumed-item-409/receipt-for-unlocated/serial-number-fallback/audit-scan-event, scan-events auth-required + filter-by-result, cross-tenant workstation isolation, MCP get_workstation_status returns data); full test suite 666 tests (664 green, 2 pre-existing flakes in UpgradeFlowTests + WorkflowTests) |
| 3.35    | 2026-04-27 | **Phase 24 SPC & Capability Analysis implemented**: `SpcChart` entity (ProcessId, ContentBlockId, Name, ChartType enum XbarR/XbarS/IndividualMR/P/NP/C/U, SubgroupSize, ControlLimitSource enum Calculated/Manual, UCL/LCL/CL/RangeUCL/RangeLCL/RangeCL, TargetCpk, LSL/USL, IsActive); `SpcDataPoint` entity (SpcChartId, StepExecutionId, Value, SubgroupIndex, CapturedAt); `OutOfControlRule` enum (Rule1_BeyondThreeSigma, Rule2_TwoOfThreeBeyondTwoSigma, Rule3_FourOfFiveBeyondOneSigma, Rule4_EightConsecutiveOneSide); `Phase_F7_SPC` EF migration; `SpcCalculationService` (A2/D3/D4/d2 SPC constants for subgroup sizes 2–10, X-bar/R-bar chart limit calculation, within-subgroup σ estimation via R̄/d₂, overall σ via sample standard deviation, Cp/Cpk from within-subgroup σ with LSL/USL, Pp/Ppk from overall σ, one-sided spec limit support, 4 Nelson Rules for out-of-control detection); `SpcController` (paginated list with processId/active filters, CRUD, data points CRUD with auto-incrementing SubgroupIndex, calculate endpoint returning full SpcCalculationResultDto, dashboard endpoint with all active charts + computed Cp/Cpk/OOC counts); `SpcDtos` (SpcChartDto/SummaryDto, CreateSpcChartDto/UpdateSpcChartDto, SpcDataPointDto/AddSpcDataPointDto, SpcCalculationResultDto, SpcOutOfControlPointDto, SpcSubgroupDto); ApiClient 10 SPC methods; `SpcDashboard.razor` (/spc — KPI cards: active charts/OOC count/worst Cpk/total data points, charts table with Cpk badges color-coded red<1.0/amber<1.33/green, OOC indicators, create chart modal with process picker/chart type/subgroup size/spec limits); `SpcChartDetail.razor` (/spc/{id} — breadcrumb, spec limits card, capability indices panel with X-bar/R-bar/σ/Cp/Cpk/Ppk, calculated control limits display, Nelson rule violation table with rule badges, data points table); SPC Charts link in Quality NavMenu section; MCP `get_spc_status` tool (active charts with Cpk and OOC summary, optional process/OOC-only filters); MCP `get_process_capability` tool (detailed capability analysis with control limits, spec limits, capability indices with assessment labels, Nelson rule violations); MCP server version 3.1 with 30 tools total; 25 new `SpcTests` (chart CRUD, update, delete, not-found, paginated list, dashboard, data points empty list, calculate insufficient data, calculation service basic values, no-spec-limits null capability, empty values, Rule1 OOC detection, subgroup grouping, invalid chart type, invalid process, invalid subgroup size, filter by process, auth required, cross-tenant isolation, MCP get_spc_status, MCP get_process_capability missing/nonexistent, Pp/Ppk with spec limits, one-sided LSL/USL); full test suite 642 tests (641 green, 1 pre-existing flake in UpgradeFlowTests) |
| 3.34    | 2026-04-27 | **Phase 17 Standards Conformance Management implemented**: `StandardsClause` entity (Standard, ClauseNumber, Title, RequirementSummary, IsAs9100Addition) with `ConformanceStandard` enum (Iso9001_2015/As9100RevD); `AuditProgram` entity (Name, Standard, Year, LeadAuditor, Status) with `AuditProgramStatus` enum (Planning/Active/Closed); `Audit` entity (ProgramId, AuditType, Scope, PlannedDate, ActualDate, LeadAuditor, Status) with `AuditType` enum (Internal/External/Supplier/Surveillance) and `AuditStatus` enum (Planned/InProgress/Complete); `AuditFinding` entity (AuditId, ClauseId, FindingType, Description, ObjectiveEvidence, Status, ActionItemId FK) with `FindingType` enum (MajorNonconformance/MinorNonconformance/Observation/OpportunityForImprovement) and `FindingStatus` enum (Open/CorrectiveActionRaised/Closed); `ClauseEvidenceLink` many-to-many entity (ClauseId, EntityType, EntityId, EvidenceNote, IsAutoLinked) with `ClauseEvidenceEntityType` enum (Process/QmsDocument/ControlPlan/Pfmea/ManagementReview/NonConformance); `ClauseCoverageStatus` enum (Covered/PartialCoverage/Gap/OpenMajorFinding) computed per-clause from evidence links and open findings; `Phase17_StandardsConformance` EF migration; `SeedStandardsClausesAsync` in DataSeeder — 37 ISO 9001:2015 clauses (4.1–10.3) + 10 AS9100 Rev D additions, idempotency guard on clause 4.1, auto-links QMS documents to governing clauses via `SeedClauseEvidenceLinksAsync`; `StandardsClausesController` (list with standard filter, get by ID, dashboard endpoint with coverage aggregation + open finding counts + next audit date, evidence link CRUD with duplicate check and entity name resolution); `AuditProgramsController` (paginated list with status/year filter, CRUD, activate/close lifecycle, delete guard when audits exist); `AuditsController` (paginated list with programId/status filter, CRUD, start/complete lifecycle, findings CRUD, raise-corrective-action endpoint creating ActionItem with Critical/High priority by finding type, close-finding endpoint with CA verification gate); `Phase17Dtos` (StandardsClauseDto/SummaryDto, ClauseEvidenceLinkDto/CreateDto, AuditProgramDto/SummaryDto/CreateDto/UpdateDto, AuditDto/SummaryDto/CreateDto/UpdateDto, AuditFindingDto/CreateDto/CloseDto, ConformanceDashboardDto); ApiClient 18 Phase 17 methods; `ConformanceDashboard.razor` (/conformance — KPI cards, clause coverage heatmap with clickable clause buttons, standard filter); `ClauseList.razor` (/conformance/clauses — clause table with coverage badges, detail modal with evidence links + add/delete evidence); `AuditProgramList.razor` (/audit-programs — paginated table with status/year filter, create modal, activate/close/delete actions); `AuditProgramDetail.razor` (/audit-programs/{id} — KPI cards, audits table with start/complete lifecycle, findings panel with add finding modal + raise CA + close finding, edit programme modal); Conformance NavMenu section (Dashboard, Clauses, Audit Programmes); MCP `get_conformance_status` tool (clause coverage summary, open major findings, next audit date); 21 new `ConformanceTests` (seeded clauses accessible, standard filter, get-by-id detail, dashboard aggregate, evidence link add/delete/duplicate-rejection, audit programme CRUD/lifecycle/invalid-transition/delete-blocked-with-audits/delete-succeeds-empty, audit start/complete lifecycle/invalid-transition, finding add+raise-CA, observation-CA-blocked, close-blocked-until-CA-verified, close-observation-succeeds, dashboard-major-finding-affects-coverage, auth-required, cross-tenant-isolation, MCP tool returns data); full test suite 617 tests (616 green, 1 pre-existing flake in UpgradeFlowTests) |
| 3.33    | 2026-04-22 | **F10 Plan-Gated Feature Tiers & Usage Enforcement implemented**: `PlanEnforcementService` (code-defined `PlanLimits` per subscription plan — Trial: 3 users/1 process/50 executions/mo, Starter: 25 users/1 site, Professional: 100 users/3 sites/advanced modules, Enterprise: unlimited); `IPlanEnforcementService` abstraction with `CheckAsync(PlanResource)` returning `PlanCheckResult` (Allowed/AtLimit/Blocked with upgrade prompt); `PlanResource` enum (Users/Processes/Sites/MonthlyExecutions/AdvancedModules); `UsageMeteringService` (`IUsageMeteringService.IncrementAsync` — upsert monthly `UsageMetric` rows); `PlanEnforcementDtos` (PlanCheckResultDto, PlanLimitsDto, PlanUsageSummaryDto); `BillingController` extended with `GET /api/billing/plan` (usage summary with current counts vs limits), `GET /api/billing/plan/check/{resource}` (per-resource limit check), `POST /api/billing/sync-features` (syncs TenantFeatureFlags from current plan); `AuthController.Register` enforces user limit (402 with upgrade prompt); `ProcessesController.Create` enforces process limit; `JobsController.Create` enforces monthly execution limit; `JobsController.Complete` increments JobExecutions usage metric; `PfmeasController.ExportPdf` + `ControlPlansController.ExportPdf` increment PdfExports metric; `StripeWebhookController.HandlePaymentSucceeded` auto-syncs feature flags on plan activation; `SyncFeatureFlagsForPlan` static method propagates plan→flags (Professional/Enterprise enables advanced modules + production/warehouse/training tools); legacy tenants without subscription records bypass enforcement gracefully; 14 new `PlanEnforcementTests` (Trial blocked at 4th user, Starter allows beyond Trial limit, Trial blocked at 2nd process, Starter unlimited processes, Professional auto-enables advanced modules, usage metering on job completion, upgrade prompt in 402 body, Enterprise has no limits, plan check endpoint, plan change propagates flags, Trial blocked at execution limit, PDF export increments metric, billing plan requires auth, cross-tenant plan isolation); full test suite 583 tests all green |
...
**Status:** ✅ Implemented 2026-05-04 (version 3.36). See changelog for full details.
...
**Goal:** Give leadership a strategic planning layer on top of the operational system. Based on Kaplan & Norton's Balanced Scorecard ("Conceptual Foundations of the Balanced Scorecard", Kaplan, HBS working paper 10-074): translate mission and vision into strategic objectives organized into perspectives, attach measurable indicators with targets to each objective, map cause-and-effect relationships between objectives (strategy map), and drive execution through initiatives. The differentiator over standalone BSC tools: objectives link directly to the Processes and Workflows that realize them, and measures can be fed live from operational data already captured by the platform instead of being re-keyed into a slide deck.

**Conceptual grounding (what the Kaplan paper prescribes and how it maps here):**

- **Four perspectives** — Financial, Customer, Internal Business Process, Learning & Growth — seeded as defaults but editable (Kaplan: the four perspectives are "a template, not a straitjacket"; non-profits and regulated industries re-label or add perspectives).
- **Objectives → Measures → Targets → Initiatives** — each perspective holds strategic objectives; each objective carries one or more measures with a baseline, target, and RAG thresholds; initiatives are the discrete actions launched to close the gap to target.
- **Strategy map / cause-and-effect chains** — Learning & Growth objectives drive Internal Process improvement, which drives Customer outcomes, which drive Financial results. Modeled as directed links between objectives rendered as a layered graph (perspectives as rows).
- **Strategy as a continual process** — the scorecard is reviewed on a cadence; integrates with Management Reviews (Phase 15) as a standing agenda input.

**50a — Entities, migration, CRUD API**

- `Scorecard` (Code auto-generated `BSC-NNN`, Name, MissionStatement, VisionStatement, StrategyNotes, Status enum Draft/Active/Archived, OwnerOrgUnitId FK → OrgUnit nullable).
- `ScorecardPerspective` (ScorecardId FK, Name, Description, SortOrder). On scorecard creation the four Kaplan-Norton perspectives are seeded; users may rename, reorder, add, or remove.
- `StrategicObjective` (PerspectiveId FK, Code auto-generated `OBJ-NNN`, Name, Description, OwnerOrgUnitId FK → OrgUnit nullable, Status enum Proposed/Active/Achieved/Retired, TargetDate nullable, SortOrder).
- `ObjectiveMeasure` (ObjectiveId FK, Name, Units, Direction enum HigherIsBetter/LowerIsBetter/TargetIsBest, BaselineValue decimal?, TargetValue decimal, GreenThreshold/RedThreshold decimal?, SourceType enum — see 50b, SourceEntityId Guid?, SourceParameter string? for source-specific config).
- `MeasureSnapshot` (MeasureId FK, Value decimal, CapturedAt, CaptureSource enum Manual/Auto, Note) — append-only history powering trend sparklines (reuses `TimeSeriesChart.razor`).
- `ObjectiveCauseLink` (ScorecardId FK, SourceObjectiveId FK, TargetObjectiveId FK, Description) — strategy-map edges; no self-links, unique per directed pair, both ends in same scorecard.
- `ObjectiveProcessLink` (ObjectiveId FK, ProcessId FK nullable, WorkflowId FK nullable — exactly one set, Note) — "this existing Process/Workflow realizes this objective".
- `ScorecardsController` (`/api/scorecards`, Admin/Engineer): scorecard/perspective/objective/measure/link CRUD; `GET /{id}/status` returns the full tree with resolved current values and RAG state; `GET /{id}/strategy-map` returns nodes + edges; `POST /api/measures/{id}/snapshots` manual reading entry.
- EF migration `Phase50_BalancedScorecard`; integration tests for CRUD, link constraints, cross-tenant isolation.

**50b — Measure value resolution (reuse of existing modules)**

`MeasureValueResolver` service: `Resolve(ObjectiveMeasure) → decimal?`, switching on `MeasureSourceType`. This is where the objective's "maturity" can be recognized from operational output, per the design intent that a linked process's yield expresses how mature the objective is:

| SourceType | Resolution | Existing feature leveraged |
|---|---|---|
| `Manual` | Latest `MeasureSnapshot` value | — |
| `ProcessYield` | Good-grade output items ÷ total input items across completed jobs of the linked Process (from `PortTransaction` grades), rolling window in `SourceParameter` | Phase 5 execution traceability |
| `ProcessMaturity` | Average step maturity score (0–100) across the linked Process's steps | Phase 8 `MaturityScoringService` |
| `WorkflowThroughput` | Completed `WorkflowJob` count in window for the linked Workflow | Phase 12 workflow execution |
| `SpcCapability` | Latest Cpk of the linked `SpcChart` | Phase 24 SPC |
| `ActionCloseRate` | 30-day action item close rate (optionally scoped to an OrgUnit) | Phase 15 accountability |
| `OpenNonConformances` | Open NC count (optionally scoped to a Process) | Phase 8c NCs |
| `TrainingCompliance` | % of required competencies current vs. `ProcessTrainingRequirement` | Phase 16 training |
| `Oee` | OEE % for linked Equipment or plant-wide | Phase 29 OEE dashboard |
| `QualityCost` | Cost of quality total in window (by category) | Phase 35 CoQ |
| `PromptMetric` | Aggregated numeric prompt value (avg/min/max) via the analytics query path | Ad-hoc analytics |

- `ScorecardSnapshotService` (BackgroundService, follows the `WorkflowSchedule` poller pattern): nightly resolves every auto-sourced measure and appends a `MeasureSnapshot`, so trends accumulate without manual entry and RAG status is cheap to read.
- RAG evaluation: Green/Amber/Red from thresholds respecting `Direction`; objective status rolls up worst-of its measures; perspective and scorecard roll up counts.

**50c — Builder UI (the workflow the user steps through)**

New **Strategy** NavMenu section (Admin/Engineer, gated by new `TenantFeatureFlags.ShowStrategyTools`), containing **Balanced Scorecard**:

- `ScorecardList.razor` (`/strategy/scorecards`) — card grid with status badges, objective/measure counts, RAG summary, create modal.
- `ScorecardBuilder.razor` (`/strategy/scorecards/{id}`) — guided, stepper-style builder mirroring the OnboardingWizard pattern:
  1. **Foundation** — name, mission, vision (the paper's starting point: measures must derive from mission/strategy, not from an ad-hoc KPI list).
  2. **Perspectives** — confirm/customize the four defaults.
  3. **Objectives** — add objectives per perspective; assign OrgUnit owner.
  4. **Measures & targets** — per objective: pick a source (manual, or one of the live operational sources above via a cascading picker — e.g. choose `ProcessYield` then pick the Process), set baseline/target/thresholds.
  5. **Link the work** — attach existing Processes/Workflows (`ObjectiveProcessLink`) and spawn **initiatives** as `ActionItem`s (new `ActionItemSourceType.StrategicObjective`), so initiatives inherit the full Phase 15 assignment/verification/overdue machinery and appear in My Actions / Team Actions.
  6. **Strategy map** — draw cause-and-effect links between objectives on a layered canvas (perspectives as horizontal bands; reuses the WorkflowBuilder canvas interaction pattern).
- `ScorecardDashboard.razor` (`/strategy/scorecards/{id}/dashboard`) — the consumption view: perspective columns of objective cards with RAG dot, current vs. target, trend sparkline, linked-process maturity/yield chips, open initiative counts; drill into an objective for measure history and linked work.

**50d — Integrations & polish**

- Management Review (Phase 15): scorecard RAG summary available as a review input; review can raise initiatives directly against objectives.
- MCP tools `get_scorecard_status` / `list_at_risk_objectives` (extends the Phase 20 BYOAI surface so a company assistant can answer "how are we tracking against strategy?").
- Quality Scorecard cross-link: the existing `/quality-scorecard` page remains the operational quality view; the Balanced Scorecard is the strategic layer above it (a Financial/Internal-Process measure can source from the same data).
- CSV export of scorecard status; seeded example scorecard for onboarding/demo (`DataSeeder` method, consistent with Phase 13 content library approach).

**Build order:** 50a → 50b → 50c → 50d. 50a+50b are pure API and independently testable; 50c is the bulk of UI effort; the strategy-map canvas (step 6) can ship after the list/builder/dashboard since cause links are additive.
...
842 integration tests across all phases. Tests run against an in-memory SQLite database spun up per test run. Test files cover all controllers including Analytics, Alerts, Reports, Document Library filters, SPC, Workstations, API Keys, Suppliers, CAPA, Calibration, Gage Studies (MSA/GR&R), OEE, and Change Orders (ECO).
...
- **Phase 17 — Standards Conformance Management** ✅ `StandardsClause` seed table (37 ISO 9001:2015 + 10 AS9100 Rev D clauses); `ClauseEvidenceLink` many-to-many with auto-linking for QMS documents; `AuditProgram`/`Audit`/`AuditFinding` entities with ActionItem FK for CA tracking; `StandardsClausesController` with dashboard endpoint (coverage aggregation + clause-level heatmap data); `AuditProgramsController` (CRUD, activate/close lifecycle); `AuditsController` (CRUD, start/complete lifecycle, findings CRUD, raise-CA with ActionItem integration, close-finding with CA verification gate); `ConformanceDashboard.razor` (/conformance — KPI cards, clause-coverage heatmap); `ClauseList.razor` (/conformance/clauses — coverage table, detail modal, evidence link management); `AuditProgramList.razor` + `AuditProgramDetail.razor` (/audit-programs — full audit and finding management); MCP `get_conformance_status` tool; 21 integration tests

- **Phase 24 — SPC & Capability Analysis** ✅ `SpcChart` + `SpcDataPoint` entities; `SpcChartType` enum (7 chart types); `ControlLimitSource` enum; `OutOfControlRule` enum (4 Nelson rules); `SpcCalculationService` (A2/D3/D4/d2 constants, X-bar/R chart limits, Cp/Cpk/Pp/Ppk computation, 4 Nelson rules OOC detection); `SpcController` (paginated CRUD, data points, calculate, dashboard); `SpcDashboard.razor` + `SpcChartDetail.razor` Blazor pages; NavMenu SPC Charts link under Quality; MCP `get_spc_status` + `get_process_capability` tools; MCP v3.1 with 30 tools; 25 integration tests

- **Phase 21 — Automatic Inventory Tracking** ✅ `ApiKey` entity (SHA-256 hashed key, KeyPrefix, workstation-scoped, CreatedByUserId, IsActive, ExpiresAt); `Workstation` entity (Code, Name, FixedLocationId FK → StorageLocation); `ScanEvent` append-only entity (not BaseEntity — own Id + ScannedAt, TenantId, WorkstationId, ApiKeyId, ScannedBarcode, ItemId, TransactionId, Result, ErrorMessage); `ScanResult` enum (6 values); `InventoryReferenceType.Workstation`; `Item.Barcode`/`StorageLocation.Barcode`/`Kind.Barcode` unique nullable barcode fields; `Phase17_F7_BarcodeAndScanTables` EF migration; `ApiKeyAuthenticationHandler` (SHA-256 hash authentication, dual-scheme with JWT via combined DefaultPolicy); `WorkstationsController` (Admin CRUD with code uniqueness, location validation, active-key-guarded soft-delete, paginated list, last-scan-time); `ApiKeysController` (Admin CRUD, raw key returned once on creation, PATCH for name/active/expiry, hard-delete); `POST /api/warehouse/scan` (API key auth, barcode→Item resolution with SerialNumber fallback, item status validation, idempotent already-at-location, Transfer/Receipt creation, ScanEvent audit, webhook fire); `GET /api/warehouse/scan-events` (paginated, filtered by workstationId/result/barcode/date range); `WorkstationList.razor` (Admin page at `/admin/workstations` — workstation CRUD with location picker modal, inline expandable API key panel with generate/deactivate/delete, scan event log viewer, search and active/inactive filter, NavMenu link under Admin section); 10 ApiClient methods; MCP `get_workstation_status` tool; MCP v3.2 with 31 tools; 38 integration tests in Phase21Tests + WorkstationAdminTests

- **Phase 27 — CAPA Workflow** ✅ `CapaRecord` entity (Code CAPA-YYYY-NNN, Type Corrective/Preventive, SourceType 7 values, ProblemStatement, ContainmentAction, RCA link, PermanentCorrectiveAction, PreventiveAction, VerificationMethod/DueDate, VerifiedByUserId/At, EffectivenessReviewDate/VerifiedByUserId/At, Status 7-stage lifecycle Open→Containment→RCA→Implementation→Verification→EffectivenessReview→Closed, OwnerUserId/DisplayName, TeamMemberIds, ClosedAt); `CapaStep` entity (StepType, CompletedBy, Notes, AttachmentFileName); `CapaStatus`/`CapaType`/`CapaSourceType` enums; `ActionItemSourceType.Capa` added; `Phase27_CapaWorkflow` EF migration; `CapaController` (paginated list with status/type/sourceType/search filters, CRUD, linear lifecycle transitions with gate checks, verify with anti-self-certification, verify-effectiveness, close with effectiveness-verified gate, link-rca with Ishikawa/FiveWhys validation, steps CRUD, action items CRUD, dashboard with aggregates); NavMenu CAPA link under Quality; MCP `get_capa_status` tool; MCP v3.4 with 33 tools; 31 integration tests in CapaTests

- **Phase 25 — Supplier Quality Management (Basic)** ✅ `Supplier` entity (Code, Name, Status with lifecycle enum Pending/Approved/Conditional/Suspended/Inactive, ContactName/Email/Phone, Address, Notes, ApprovedDate, LastEvaluationDate, IsActive); `SupplierEvaluation` entity (SupplierId, EvaluationDate, QualityScore/DeliveryScore/ResponsivenessScore 0–100, computed OverallScore, EvaluatedByUserId); `SupplierStatus` enum with valid state-machine transitions; `Phase25_SupplierQualityManagement` EF migration; `SuppliersController` (paginated list with search/status/active filters, CRUD, status transition with validation, evaluations CRUD, quality dashboard endpoint with KPIs + top performers + at-risk suppliers); `Phase25Dtos` (SupplierResponseDto/SummaryDto, CreateSupplierDto/UpdateSupplierDto/UpdateSupplierStatusDto, SupplierEvaluationResponseDto/CreateDto, SupplierQualityDashboardDto); `SupplierList.razor` (/suppliers — KPI cards, status/search filter, paginated table, create/edit modals, status transition, evaluations panel); NavMenu Suppliers link under Quality section; ApiClient 9 supplier methods; MCP `get_supplier_quality_status` tool (status breakdown, supplier table with scores, at-risk list); MCP v3.3 with 32 tools; 22 integration tests in SupplierTests

- **Phase 26 — Measurement System Analysis (MSA/GR&R)** ✅ `GageStudy` entity (Name, StudyType enum GRR_Range/GRR_ANOVA/Linearity/Bias/Stability, EquipmentId FK → Equipment, ProcessId FK → Process, CharacteristicName, Tolerance, LSL, USL, NumberOfParts/NumberOfOperators/NumberOfTrials, Status enum Draft/InProgress/Complete, GrrPercent, Ndc, AcceptanceDecision); `GageStudyMeasurement` entity (GageStudyId, PartNumber, OperatorId, TrialNumber, MeasuredValue, unique composite index); `GageStudyType`/`GageStudyStatus` enums; `GrrCalculationService` (Range method with d₂/K1/K2/K3 AIAG constants for subgroup sizes 2–10, EV/AV/GRR/PV/TV decomposition, %GRR of total variation and tolerance, ndc calculation, three-tier assessment); `GageStudyController` (paginated list with status/equipment filters, CRUD, bulk measurement add with bounds validation, auto-transition Draft→InProgress, calculate with completion, dashboard); NavMenu Gage Studies link under Quality; MCP `get_msa_status` tool; MCP v3.6 with 35 tools; 23 integration tests in GageStudyTests

- **Phase 28 — Calibration Management** ✅ `CalibrationRecord` entity (EquipmentId FK → Equipment, CalibrationType enum Internal/External/Vendor, CalibrationDate, NextDueDate, CertificateNumber/FileName, Result enum Pass/Fail/Limited, PerformedBy, StandardsUsed, TemperatureHumidity, AsFound/AsLeftReading, Uncertainty, Notes); `CalibrationSchedule` entity (EquipmentId unique, IntervalDays, IntervalAdjustmentMethod enum Fixed/ReliabilityBased, ConsecutivePassCount, MaxIntervalDays, MinIntervalDays, ExtensionPercent); reliability-based interval adjustment (pass extends by ExtensionPercent capped at MaxIntervalDays, fail resets to MinIntervalDays); `CalibrationResult`/`CalibrationType`/`IntervalAdjustmentMethod` enums; `Phase28_CalibrationManagement` EF migration; `CalibrationController` (records CRUD with equipment/result filters, equipment history, schedules CRUD with duplicate guard, dashboard with due/overdue recall detection); NavMenu Calibration link under Production; MCP `get_calibration_status` tool; MCP v3.5 with 34 tools; 26 integration tests in CalibrationTests

- **Phase 32 — Change Management & ECO Workflow** ✅ `ChangeOrder` entity (Code ECO-YYYY-NNN, Type 5 values, Priority 3 values, Status 7-stage lifecycle Draft→ImpactAnalysis→Approval→Implementation→Verification→Closed/Rejected, RequestedBy/ClosedAt/RejectionReason); `ChangeOrderImpact` entity (AffectedEntityType 7 values, AffectedEntityId, ImpactDescription, MitigationPlan); `ChangeOrderApprover` entity (UserId, Decision Pending/Approved/Rejected/Abstained, Comments); `ChangeOrderTask` entity (Title, AssigneeUserId, DueDate, Status reusing ActionItemStatus, CompletedAt); `ActionItemSourceType.ChangeOrder` added; `Phase32_ChangeOrders` EF migration; `ChangeOrdersController` (paginated list with 4 filters, CRUD, lifecycle transitions with gate checks: impact required for submit, approver required for request-approval, all-approved for approve, tasks-complete for verification, reject with reason; impact/approver/task sub-resource CRUD; dashboard); NavMenu Change Orders link under Conformance section; MCP `get_change_order_status` tool (status/type/priority filter, summary with breakdowns and overdue table); MCP v3.9 with 38 tools; 38 integration tests in ChangeOrderTests

- **Phase 34 — Customer Complaint Management & Voice of Customer (VoC)** ✅ `CustomerComplaint` entity (Code auto-generated CC-YYYY-NNN, CustomerName, CustomerReference, ProductKindId FK → Kind, LotNumber, ComplaintDate, ReceivedDate, Category enum ProductDefect/Packaging/Delivery/Documentation/Service/Regulatory, Severity enum Critical/Major/Minor/Cosmetic, Description, QuantityAffected, Status 7-stage lifecycle New→UnderInvestigation→ContainmentInPlace→RootCauseIdentified→CorrectiveActionImplemented→ResponseSent→Closed, OwnerUserId/OwnerDisplayName, ResponseDueDate, ResponseSentAt, CustomerSatisfied, LinkedNonConformanceId/LinkedCapaId/LinkedSupplierId, ClosedAt); `ComplaintInvestigation` entity (CustomerComplaintId, InvestigationType enum InitialAssessment/LabAnalysis/ProcessReview/SupplierInvestigation, Findings, InvestigatedByUserId/DisplayName, InvestigatedAt); `ComplaintResponse` entity (CustomerComplaintId, ResponseType enum Acknowledgment/InterimUpdate/FinalResponse/CreditNote/Replacement, Content, SentByUserId/DisplayName, SentAt); `ComplaintStatus`/`ComplaintCategory`/`ComplaintSeverity`/`InvestigationType`/`ComplaintResponseType` enums; `ActionItemSourceType.CustomerComplaint` added; `Phase34_CustomerComplaintManagement` EF migration; `CustomerComplaintsController` (Admin/Engineer, paginated list with status/category/severity/search filters, CRUD, 7-stage lifecycle transitions with gate checks: investigation required for UnderInvestigation, linked CAPA/NC required for RootCauseIdentified, response required for ResponseSent, administrative closure from any open state; investigations CRUD; responses CRUD; action items list from Phase 15 linkage; dashboard with open/overdue/avgDaysToClose/customerSatisfactionRate/byStatus/byCategory/bySeverity); NavMenu Complaints link under Quality section; ApiClient 11 complaint methods; MCP `get_complaint_status` tool (status/category/severity filter, summary with KPIs, status/category/severity breakdowns, overdue complaints table); MCP v4.0 with 39 tools; 29 integration tests in CustomerComplaintTests

#### Partially built
*(none at this time)*

#### Not yet built
- **Phase 23 — Supplier Quality Management (SQM)** — Extends the existing Phase 10d MRB + SCAR workflow into a full supplier quality programme. `Supplier` entity (Code, Name, ApprovedStatus, QualityRating computed from incoming inspection pass rate and SCAR closure timeliness, CertificateExpiryDate, RiskTier enum Low/Medium/High/Critical); `SupplierCertificate` entity (CertificateType enum ISO9001/AS9100/IATF16949/NADCAP/Custom, IssueDate, ExpiryDate, DocumentFileName); `IncomingInspection` entity linked to `InventoryTransaction.Receipt` (InspectionResult enum Accept/ConditionalAccept/Reject, SamplingPlan, LotSize, SampleSize, DefectsFound); automatic `NonConformance` creation on Reject; automatic SCAR assignment to supplier on recurring rejects (configurable threshold); `SupplierScorecard` aggregation endpoint (acceptance rate, average lead time vs quoted, SCAR count/closure rate, on-time delivery %); `SupplierList.razor` (/suppliers — approved supplier registry with risk tier badges and expiring-cert alerts); `SupplierDetail.razor` (scorecard dashboard, certificate management, incoming inspection history, linked SCARs); `IncomingInspectionList.razor` (/incoming-inspection — lot-by-lot results table with pass/fail trend); MCP `get_supplier_scorecard` tool; MCP `list_supplier_quality_alerts` tool (expiring certs, trending rejects, overdue SCARs). **Market fit:** regulated manufacturers (aerospace, medical, automotive) are required to maintain an Approved Supplier List and demonstrate incoming inspection rigour — this is table-stakes for AS9100 clause 8.4 and IATF 16949 clause 8.4.2 compliance and is a major gap in many QMS-only platforms.
- **Phase 25 — Customer Portal & FAIR/PPAP Document Package** — A read-only external portal for customer quality representatives to view conformance evidence without system access. (moved to Not yet built below)
- **Phase 26 — Measurement System Analysis (MSA/GR&R)** ✅ *(implemented — see completed section above)*
- **Phase 27 — CAPA Workflow** ✅ *(implemented — see completed section above)*
- **Phase 28 — Calibration Management** ✅ *(implemented — see completed section above)*

- **Phase 25 — Customer Portal & FAIR/PPAP Document Package** — A read-only external portal for customer quality representatives to view conformance evidence without system access. `CustomerPortal` entity (CustomerName, ContactEmail, AccessToken hashed, ExpiresAt, AllowedProcessIds scope); `DocumentPackage` entity (ProcessId, PackageType enum FAIR/PPAP/CoC/CustomAuditPack, GeneratedAt, PdfBlobPath, IncludedSections JSON list); `FairGenerator` service producing an AS9102 First Article Inspection Report PDF from process execution data (balloon drawings linked to Control Plan characteristics, measured values from prompt responses, pass/fail disposition, traceability to job/serial/lot); `PpapGenerator` service producing an AIAG PPAP submission package (Process Flow Diagram from process model, PFMEA PDF, Control Plan PDF, dimensional results from SPC/prompt data, material certs from Kind documents, MSA/GR&R placeholder); `CustomerPortalController` (token-based access: GET portal landing, GET allowed processes, GET document packages, GET package PDF download); `CustomerPortalSetup.razor` (Admin page: create portal link, select processes, generate token, set expiry); `DocumentPackageList.razor` (generate FAIR/PPAP packages for a process, view history); integration with Phase 17 conformance data (FAIR can reference clause evidence); integration with Phase 24 SPC data (capability reports included in PPAP). **Market fit:** aerospace (AS9102 FAIR is mandatory for every new part), automotive (PPAP is mandatory for production part approval), and contract manufacturers all need to produce formal quality evidence packages for customers. Today this is a painful manual assembly of spreadsheets, PDFs, and inspection reports — automating it from live system data is a high-value differentiator that directly reduces the cost of new-part qualification.

- **Phase 29 — OEE (Overall Equipment Effectiveness) Dashboard** ✅ *(implemented — see completed section above)*

- **Phase 30 — Electronic Batch Record (EBR) & Device History Record (DHR)** — Formal batch-level documentation package for regulated industries (pharma, medical devices, food). `BatchRecord` entity (BatchId FK → Batch, ProcessId, ProductCode, LotNumber, Status enum InProgress/PendingReview/Approved/Released, ReviewedBy, ReviewedAt, ReleasedBy, ReleasedAt, DeviationCount); `BatchRecordSection` entity (BatchRecordId, SectionType enum MaterialVerification/InProcessChecks/EnvironmentalMonitoring/YieldReconciliation/PackagingVerification/FinalRelease, Data JSON, SignedBy, SignedAt); auto-population: when a batch-tracked Job completes, the system generates a `BatchRecord` from all execution data (prompt responses, port transactions, non-conformances, deviations, operator sign-offs, environmental readings); `BatchRecordController` (GET list with status filter, GET detail with all sections, POST review with electronic signature, POST release with dual-signature requirement, GET PDF export with 21 CFR Part 11 compliant audit trail); `BatchRecordList.razor` (/batch-records — pending review queue with deviation count badges, released batches archive); `BatchRecordDetail.razor` (/batch-records/{id} — section-by-section review with approve/reject per section, yield reconciliation calculator, deviation summary, electronic signature capture); `DhrGenerator` service producing an FDA-compliant Device History Record PDF from batch record data; integration with Phase 19 warehouse (material lot traceability from receipt through consumption); integration with Phase 8d ExecutionWizard (prompt responses feed batch record sections). **Market fit:** FDA 21 CFR 211 (pharma) and 21 CFR 820.184 (medical devices) require complete batch/device history records for every production lot. Most manufacturers maintain these as paper binders or disconnected electronic forms — the #1 FDA 483 observation category is incomplete or missing batch records. Automating EBR/DHR from live execution data eliminates this compliance risk and reduces batch release cycle time from days to hours.

- **Phase 31 — Predictive Quality Analytics & ML-Driven Alerts** — Machine learning layer that analyses historical execution data to predict quality issues before they occur. `PredictiveModel` entity (Id, Name, ModelType enum DriftDetection/FailurePrediction/AnomalyDetection, TargetContentBlockId FK → StepTemplateContent, TrainingDataPoints int, LastTrainedAt, ModelBlobPath, IsActive, AccuracyMetric decimal); `PredictiveAlert` entity (ModelId, StepExecutionId, AlertType enum DriftWarning/FailureProbability/Anomaly, Confidence decimal 0–1, PredictedValue decimal, ActualValue decimal, Acknowledged bool); `DriftDetectionService` (monitors running mean and variance of NumericEntry prompt responses over a sliding window, fires alert when drift exceeds configurable sigma threshold — catches process drift before SPC control limits are breached); `FailurePredictionService` (logistic regression trained on historical prompt response vectors → NonConformance outcomes, predicts probability of NC for the current execution based on upstream prompt responses already captured); `AnomalyDetectionService` (isolation forest or z-score ensemble on multivariate prompt response profiles, flags executions whose measurement pattern is statistically unusual even if individual values are in-spec); `PredictiveController` (GET `/api/predictive/models` CRUD, POST `/api/predictive/models/{id}/train` triggers background training job, GET `/api/predictive/alerts` paginated with confidence threshold filter, POST `/api/predictive/alerts/{id}/acknowledge`); `PredictiveAlertsDashboard.razor` (/predictive — active model summary cards, alert feed with confidence badges, trend charts showing predicted vs actual, model accuracy over time); integration with Phase 24 SPC (drift detection supplements Nelson rules with earlier warning); integration with Phase 8d ExecutionWizard (inline warning banner when failure prediction exceeds threshold during active execution); MCP `get_predictive_alerts` tool. **Market fit:** traditional SPC is reactive — it detects out-of-control conditions after they happen. Industry 4.0 and smart manufacturing initiatives demand predictive quality: catching drift and predicting failures before scrap is produced. This differentiates the platform from both legacy QMS tools (which have no analytics) and standalone SPC packages (which lack the process context to build predictive models). Manufacturers investing in digital transformation specifically seek platforms that offer "quality intelligence" — actionable predictions derived from their own production data, not just charts of what already went wrong.

- **Phase 32 — Change Management & Engineering Change Order (ECO) Workflow** ✅ *(implemented — see completed section above)*

- **Phase 33 — Real-Time Shop Floor Display & Andon System** — Large-format display mode showing live production status, designed for wall-mounted monitors at workstations and production meeting areas. `DisplayLayout` entity (Code, Name, Type enum ProductionBoard/AndonBoard/QualityBoard/KpiBoard, RefreshIntervalSeconds, LayoutConfig JSON defining widget placement); `AndonEvent` entity (WorkstationId or EquipmentId, EventType enum Help/Quality/Material/Maintenance/Safety, Status enum Active/Acknowledged/Resolved, RaisedByUserId, RaisedAt, AcknowledgedByUserId, AcknowledgedAt, ResolvedAt, ResponseTimeSeconds computed); `AndonController` (raise event via simple POST suitable for physical Andon button integration, acknowledge, resolve, active events list, response time analytics); `DisplayController` (GET `/api/displays/{code}/data` returns computed widget data for a display layout, suitable for polling or SSE; layouts CRUD); `ShopFloorDisplay.razor` (/display/{code} — full-screen kiosk mode with auto-refresh, no sidebar/nav, large typography, color-coded status, auto-cycling between configured widgets); display widgets: WIP board (from Phase 11 production data), Andon status (active calls with escalation timer), OEE gauges (from Phase 29 when built, placeholder until then), quality alerts (open NCs and SPC OOC), schedule adherence (jobs on-time vs late), shift production counter; `AndonBoard.razor` (/andon — Andon event management for supervisors: active events, response metrics, raise/acknowledge/resolve); `DisplayAdmin.razor` (/admin/displays — layout configuration, widget selection, assignment to physical screens); physical integration: REST API compatible with Raspberry Pi kiosk browsers, industrial PCs, and Android tablets; `AndonEventDto`, `RaiseAndonEventDto`, `DisplayLayoutDto`; MCP `get_andon_status` tool. **Market fit:** every lean manufacturing facility has visual management boards — traditionally whiteboards or expensive proprietary Andon systems (Werma, Banner Engineering, FactoryTalk). Modern manufacturers want digital boards driven by live data, not manually updated. The Andon response-time metric is a key lean KPI tracked in Toyota Production System implementations. Providing this as a built-in feature (rather than requiring a separate manufacturing intelligence platform like Tulip or L2L) dramatically reduces the total cost of digital shop floor transformation. This is the feature that makes the platform visible to every person on the factory floor, not just engineers at desks.

- **Phase 34 — Customer Complaint Management & Voice of Customer (VoC) Integration** ✅ *(implemented — see completed section above)*

- **Phase 35 — Cost of Quality (CoQ) Tracking & Financial Impact Analysis** — Attaches financial costs to quality events (scrap, rework, warranty, inspection) and provides executive-level reporting on quality economics. `QualityCost` entity (SourceType enum Scrap/Rework/Warranty/InspectionLabor/ExternalFailure/PreventionCost/AppraisalCost, SourceEntityId FK → NonConformance/CapaRecord/CustomerComplaint/MaintenanceTask, Amount decimal, Currency string, CostCategory enum Prevention/Appraisal/InternalFailure/ExternalFailure per PAF model, KindId FK → Kind optional for product-level attribution, JobId FK → Job optional, Description, RecordedByUserId, RecordedAt); `QualityCostRule` entity (TriggerEvent enum NcCreated/NcScrapped/NcReworked/CapaOpened/ComplaintReceived/InspectionPerformed, DefaultCostFormula text — e.g. `quantity * unit_cost` or fixed amount, IsActive); auto-cost creation: when a NonConformance disposition is set to Scrap, system auto-creates a QualityCost entry using the Kind's unit cost × quantity affected; rework costs computed from step execution duration × labor rate; `QualityCostController` (CRUD, manual cost entry, auto-cost configuration, CoQ dashboard endpoint with PAF breakdown, trend over time, cost-per-unit-shipped, top cost drivers Pareto, product-level CoQ ranking); `CoqDashboard.razor` (/quality-costs — executive KPI cards: total CoQ this month/quarter/year, CoQ as % of revenue configurable target, PAF category breakdown donut chart, 12-month trend line, top 10 cost drivers by Kind/Process/FailureMode, cost per unit shipped trend); `CoqDetail.razor` (/quality-costs/entries — individual cost entries with filters by category/source/product/date, manual entry for costs not captured automatically); `QualityCostRules.razor` (/admin/quality-cost-rules — configure auto-cost triggers and default amounts/formulas); integration with Phase 8c NonConformance (scrap/rework costs); integration with Phase 27 CAPA (CAPA implementation costs); integration with Phase 34 complaints (warranty and external failure costs); integration with Phase 15 Management Review (CoQ summary as management review input); MCP `get_cost_of_quality` tool (PAF breakdown, trend, top drivers). **Market fit:** Cost of Quality is the language executives understand — while engineers focus on PPM and Cpk, leadership makes investment decisions based on the financial impact of poor quality. ISO 9004 recommends CoQ analysis, and IATF 16949 clause 6.1.2.1 specifically requires the organization to include warranty cost in quality objectives. The #1 challenge quality managers face is justifying improvement investments — CoQ data provides the ROI calculation. Most QMS platforms track quality events but not their financial impact, forcing quality managers to maintain separate spreadsheets for management presentations. Embedding CoQ into the platform means every NCR, CAPA, and complaint automatically accumulates financial evidence for improvement business cases.

- **Phase 36 — Skill-Based Work Assignment & Intelligent Scheduling** — Matches operator competencies, certifications, and availability to workstation/process requirements for optimal job routing. `OperatorAvailability` entity (UserId, ShiftDefinitionId FK → ShiftDefinition, Date, AvailabilityType enum Available/PTO/Training/Modified, StartOverride TimeOnly?, EndOverride TimeOnly?); `ProcessSkillRequirement` entity (ProcessId FK → Process, SkillType enum Certification/Training/Experience/PhysicalCapability, SkillCode FK → CompetencyRecord or custom, MinimumLevel enum Awareness/Competent/Proficient/Expert, IsHardRequirement bool — hard requirements block assignment, soft requirements generate warnings); `WorkAssignment` entity (JobId FK → Job, StepExecutionId FK → StepExecution, AssignedUserId, AssignedAt, AssignedByMethod enum Manual/AutoBestFit/AutoRoundRobin/AutoLeastLoaded, Score decimal — fitness score explaining why this operator was chosen, OverrideReason text — if supervisor overrode auto-assignment); `AssignmentEngine` service (scores candidate operators by: competency match weighted × proficiency level, workstation proximity from floor plan, current workload balance, recent error rate on this process, shift availability; returns ranked list with explanation); `CapacityView` DTO (per-shift per-workstation view: required competencies × available qualified operators × planned job demand → capacity sufficiency indicator with bottleneck identification); `WorkAssignmentController` (GET `/api/assignments/suggest` returns ranked operator list for a job with fitness scores, POST `/api/assignments` create assignment, GET `/api/capacity` shift-level capacity view, GET `/api/capacity/gaps` identifies understaffed shifts/skills); `WorkAssignment.razor` (/assignments — supervisor assignment board: unassigned jobs queue, operator availability grid, drag-and-drop or auto-assign, competency-mismatch warnings); `CapacityPlanner.razor` (/capacity — weekly/monthly capacity calendar with color-coded sufficiency, drill-down to skill gaps, training needs identification); integration with Phase 16 Training (expired competencies block assignment); integration with Phase 12 OrgUnit (team membership defines assignment pool); integration with Phase 22 Factory Design (workstation proximity weighting); integration with Phase 11 Production (scheduling constraints from equipment availability); MCP `get_capacity_gaps` tool. **Market fit:** manufacturing scheduling is the #1 operational pain point — supervisors spend hours daily matching operators to workstations based on tribal knowledge of who is trained for what. When a key operator calls in sick, the scramble to find qualified replacements delays production. Workforce management in manufacturing is currently handled by paper skill matrices and supervisor memory. Skill-based auto-routing is the bridge between MES (which tracks what happens) and APS (which plans what should happen) — it's the feature that turns the platform from a quality/compliance tool into an operational intelligence system. Companies with 50+ operators cite this as their #1 need when evaluating digital manufacturing platforms.

- **Phase 37 — Process Digital Twin & What-If Simulation** — Monte Carlo simulation engine that models process variability and predicts quality/throughput outcomes for proposed changes before implementation. `SimulationScenario` entity (Name, Description, BaseProcessId FK → Process, Status enum Draft/Running/Complete/Failed, CreatedByUserId, CreatedAt, CompletedAt, Iterations int default 10000); `SimulationOverride` entity (ScenarioId, TargetType enum StepDuration/PromptValue/RejectRate/EquipmentDowntime/OperatorSpeed, TargetStepId FK → ProcessStep optional, Distribution enum Normal/Uniform/Triangular/Exponential/Historical, Param1 decimal — mean or min, Param2 decimal — stddev or max, Param3 decimal — mode for triangular); `SimulationResult` entity (ScenarioId, MetricType enum Throughput/CycleTime/FirstPassYield/Cpk/WipLevel/BottleneckUtilization/CostPerUnit, P5 decimal, P25 decimal, Median decimal, P75 decimal, P95 decimal, Mean decimal, StdDev decimal); `SimulationStepResult` entity (ScenarioId, ProcessStepId, AvgDuration decimal, UtilizationPct decimal, QueueTimePct decimal, RejectRatePct decimal); `SimulationEngine` service (discrete-event Monte Carlo: models each process step as a stochastic activity with configurable distributions derived from historical prompt response data and step execution durations; models serial/parallel step flows from process graph; models reject/rework loops from historical NC rates; models equipment downtime from Phase 11 DowntimeRecord distributions; outputs throughput, cycle time, yield, WIP, and per-step utilization distributions); `HistoricalDistributionFitter` service (fits Normal/Lognormal/Weibull to historical execution data for a given process, returns best-fit distribution parameters — used as defaults for simulation overrides); `SimulationController` (CRUD scenarios, add overrides, run simulation as background job, get results, compare two scenarios side-by-side); `SimulationList.razor` (/simulations — scenario cards with status badges, create from process with auto-fitted distributions, clone for variant comparison); `SimulationDetail.razor` (/simulations/{id} — override editor per step with distribution visualizer, run button, results dashboard with histograms and confidence intervals, step-level utilization heatmap, comparison mode with baseline vs proposed change highlighting); integration with Phase 32 ECO (change order impact analysis can run simulation to predict quality/throughput effect of proposed change); integration with Phase 29 OEE (simulation results predict OEE impact of changes); integration with Phase 24 SPC (historical Cpk data feeds simulation distributions); MCP `run_simulation` tool (create and execute a quick scenario, return key metrics). **Market fit:** process engineers today make changes based on experience and hope — they tighten a tolerance, add an inspection step, or change a supplier, then wait weeks to see if quality improved or throughput dropped. Digital twin simulation is the killer feature of Industry 4.0 that lets engineers predict outcomes before committing changes to the shop floor. Enterprise platforms (Siemens Tecnomatix, Dassault DELMIA) offer this but cost $100K+ per seat and require dedicated simulation engineers. Providing lightweight what-if simulation embedded in the process model — using the system's own historical data for distribution fitting — democratizes simulation for the 95% of manufacturers who can't afford enterprise digital twin platforms. This is the feature that transforms the platform from "quality system that records what happened" to "operations intelligence platform that predicts what will happen."

- **Phase 38 — Environmental Monitoring & Clean Room Compliance** — Continuous environmental condition tracking for temperature, humidity, particulate counts, and differential pressure — critical for medical device, semiconductor, and pharmaceutical manufacturing. `EnvironmentalSensor` entity (Code, Name, SensorType enum Temperature/Humidity/Particulate/DifferentialPressure/Vibration, LocationId FK → StorageLocation, EquipmentId FK → Equipment optional, Unit, LSL, USL, IsActive, PollingIntervalSeconds); `EnvironmentalReading` append-only entity (SensorId, Value, ReadingAt, IsOutOfSpec computed, AcknowledgedByUserId, AcknowledgedAt); `CleanRoomClass` entity (Name, Standard enum ISO14644/FedStd209E, Class string, MaxParticleCount per size, TemperatureRange, HumidityRange); `EnvironmentalExcursion` entity (SensorId, StartedAt, EndedAt, PeakValue, Duration, Status enum Active/Acknowledged/Investigated/Closed, LinkedNonConformanceId, LinkedCapaId); auto-excursion detection: when a reading exceeds sensor LSL/USL, an excursion opens and optionally auto-creates a NonConformance (configurable per sensor); auto-close when readings return in-spec; `EnvironmentalController` (sensor CRUD, readings ingestion endpoint suitable for IoT gateway POST, readings query with time range, excursion list with status filter, dashboard with real-time sensor grid, configurable alerting thresholds); `EnvironmentalDashboard.razor` (/environmental — live sensor status grid with green/amber/red indicators, 24h sparkline per sensor, active excursion alerts, clean room compliance status per area); `SensorDetail.razor` (/environmental/sensors/{id} — readings time-series chart, excursion history, configuration); IoT integration: REST API compatible with MQTT gateways (AWS IoT, Azure IoT Hub) via simple HTTP POST with API key auth; integration with Phase 19 Warehouse (monitor storage conditions for material with shelf-life requirements); integration with Phase 30 EBR (environmental readings as batch record evidence); integration with Phase 17 Conformance (links to ISO 14644 clause evidence); MCP `get_environmental_status` tool (sensor summary, active excursions, out-of-spec alerts). **Market fit:** FDA 21 CFR 211.46 (pharma), ISO 14644 (clean rooms), and ISO 13485 (medical devices) mandate documented environmental monitoring with defined action limits. Most manufacturers use standalone data loggers (Vaisala, Rotronic, Dickson) that produce paper charts or CSVs disconnected from their quality system — when an excursion occurs, the link between environmental deviation and affected product batches is manual and error-prone. Embedding environmental monitoring into the process/quality platform creates automatic traceability from environmental excursions to affected batches, NCRs, and CAPAs. This is a high-value differentiator for any manufacturer in a regulated clean room environment, where a missed excursion can result in full lot rejection or FDA warning letters.

- **Phase 39 — Layered Process Audit (LPA) & Gemba Walk Digitization** — Structured observation-based audit system where leaders at all levels verify process adherence at the point of work. `LpaChecklist` entity (ProcessId FK → Process, Name, LayerLevel enum Operator/Supervisor/Manager/Director, Frequency enum PerShift/Daily/Weekly/Monthly, Questions JSON array of {question, expectedAnswer, isHighImpact}, IsActive); `LpaAudit` entity (ChecklistId, AuditorUserId, AuditedAreaId FK → OrgUnit, ScheduledDate, CompletedAt, Status enum Scheduled/InProgress/Complete/Missed, OverallResult enum Conforming/NonConforming/NotApplicable); `LpaResponse` entity (AuditId, QuestionIndex, Response enum Conforming/NonConforming/NA, Notes, PhotoFileName, LinkedActionItemId); auto-scheduling: system generates LPA audit assignments based on checklist frequency and layer level, distributes across qualified auditors in the OrgUnit, tracks completion rate; missed audit escalation: if an audit passes its scheduled date without completion, system creates escalation notification and marks as Missed; trend detection: repeated non-conformances on the same question across multiple audits auto-trigger a CAPA recommendation; `LpaController` (checklist CRUD, audit schedule generation, audit execution with response capture, layered completion dashboard, non-conformance trending by question/area/process, missed audit tracking); `LpaDashboard.razor` (/lpa — completion rate gauges per layer, this-week audit schedule with status, non-conformance heat map by area, trending issues, missed audit alerts); `LpaAuditForm.razor` (/lpa/audit/{id} — mobile-optimized checklist execution with one-tap conform/non-conform, photo capture for evidence, auto-create action items from non-conformances); `LpaAdmin.razor` (/admin/lpa — checklist builder, frequency/layer configuration, auditor pool management); integration with Phase 12 OrgUnit (audit assignments flow through department hierarchy); integration with Phase 27 CAPA (trending non-conformances trigger CAPA); integration with Phase 15 Action Items (non-conforming responses create action items); integration with Phase 33 Shop Floor Display (LPA completion status as display widget); MCP `get_lpa_status` tool (completion rates by layer, trending non-conformances, missed audits). **Market fit:** Layered Process Audits are a cornerstone of IATF 16949 (clause 9.2.2.4 requires manufacturing process audits) and are mandated by virtually every automotive OEM's customer-specific requirements (Ford, GM, Stellantis, Toyota all require documented LPA programs). The Gemba walk — leaders observing actual work at the point of production — is fundamental to lean manufacturing and Toyota Production System. Currently, 80%+ of manufacturers run LPA programs on paper clipboards or generic checklist apps (iAuditor, GoAuditor) completely disconnected from their quality system. This means non-conformances found during Gemba walks never flow into the corrective action system, and trending analysis is impossible. Digitizing LPA within the quality platform creates the direct link from observation → action item → CAPA → process improvement that lean manufacturing demands. This is the feature that demonstrates daily management discipline to automotive OEM auditors — "show me your last 30 days of LPA data" is a standard audit request.

- **Phase 40 — Risk Register & FMEA-Driven Risk Management** — Unified organizational risk register that aggregates risk signals from across the platform (PFMEA RPNs, SPC out-of-control events, overdue CAPAs, supplier quality degradation, environmental excursions) into a single prioritized view for management decision-making. `RiskRegister` entity (Code auto-generated RISK-NNN, Title, Description, Category enum ProcessRisk/ProductRisk/SupplierRisk/ComplianceRisk/EquipmentRisk/EnvironmentalRisk, Likelihood enum Rare/Unlikely/Possible/Likely/AlmostCertain with score 1–5, Consequence enum Negligible/Minor/Moderate/Major/Catastrophic with score 1–5, InherentRiskScore computed Likelihood×Consequence, MitigationPlan, ResidualLikelihood, ResidualConsequence, ResidualRiskScore computed, Status enum Open/Mitigated/Accepted/Closed, OwnerId FK → User, ReviewDueDate, LastReviewedAt, SourceType enum Manual/PfmeaHighRPN/SpcOOC/CapaRecurring/SupplierDegradation/CalibrationOverdue/EnvironmentalExcursion, SourceEntityId optional); `RiskMitigationAction` entity (RiskRegisterId, ActionItemId FK → ActionItem, TargetResidualScore, VerifiedEffective bool); `RiskAutoDetectionService` (BackgroundService that periodically scans for: PFMEA failure modes with RPN > configurable threshold and no completed action → creates ProcessRisk; SPC charts with multiple OOC events in 7 days → creates ProcessRisk; same root cause appearing in 3+ CAPAs → creates recurring risk; supplier evaluation score dropping below threshold → creates SupplierRisk; calibration overdue > 30 days → creates EquipmentRisk; creates draft risks for human review, never auto-closes); `RiskScoringMatrix` configurable 5×5 risk matrix with organization-defined color thresholds (Low/Medium/High/Extreme); `RiskController` (CRUD, risk assessment with likelihood/consequence update, add mitigation actions, close with residual score, auto-generated risks queue for review, aggregate dashboard with risk heatmap, top-10 open risks, risk trending over time); `RiskDashboard.razor` (/risks — interactive 5×5 risk heatmap matrix with risk count per cell, top open risks table sorted by inherent score, risk category breakdown, overdue-review alerts, auto-detected risks pending review queue); `RiskDetail.razor` (/risks/{id} — risk assessment panel with before/after scores, mitigation action plan linked to ActionItems, source traceability showing originating PFMEA/SPC/CAPA, review history); `RiskMatrix.razor` (/risks/matrix — configurable likelihood×consequence grid with threshold editor, drag risks between cells for re-assessment); integration with Phase 7 PFMEA (high-RPN failure modes auto-populate risk register); integration with Phase 24 SPC (recurring OOC patterns generate risk entries); integration with Phase 27 CAPA (recurring CAPAs indicate systemic risk); integration with Phase 25 Suppliers (score degradation triggers supplier risk); integration with Phase 28 Calibration (overdue calibrations → equipment risk); integration with Phase 15 Management Review (risk register is a required management review input per ISO 9001 clause 6.1); MCP `get_risk_status` tool (risk heatmap summary, top risks, auto-detected pending review, overdue reviews). **Market fit:** ISO 9001:2015 made risk-based thinking a foundational requirement (clauses 6.1, 4.4.1(f)), and AS9100 Rev D strengthened it further with explicit risk management requirements (clause 8.1.1 operational risk assessment). IATF 16949 requires specific risk analysis for product and process (clause 6.1.2.1, 6.1.2.2). Most manufacturers maintain risk registers in Excel or SharePoint, manually updated quarterly for management review — by the time a risk is documented, the damage is done. The platform already captures the raw signals (PFMEA scores, SPC data, CAPA patterns, supplier scores, calibration status) that constitute manufacturing risk. Automatically surfacing these as actionable risk entries — without requiring manual data gathering — transforms risk management from a compliance checkbox into a genuine early warning system. This is the feature that connects operational data to strategic decision-making, answering the executive question "where should we invest to prevent the next quality failure?" with data-driven evidence rather than gut feel.

- **Phase 41 — Tool Crib & Tooling Lifecycle Management** — Tracks cutting tools, fixtures, jigs, gauges, and consumable tooling from procurement through end-of-life, linking tool usage to process performance and quality outcomes. `ToolCribItem` entity (Code, Name, ToolType enum CuttingTool/Fixture/Jig/Gauge/Die/Mold/Consumable, KindId FK → Kind optional for items tracked as Kinds, CurrentLocationId FK → StorageLocation, Status enum Available/InUse/Regrind/Repair/Condemned/OnOrder, MaxLifeUnits int — cycles, hits, meters, or minutes depending on type, CurrentLifeUnits int, RegrindCount int, MaxRegrinds int, CostPerUnit decimal, VendorId FK → Supplier optional, LeadTimeDays int, MinStockLevel int, ReorderPoint int); `ToolUsageLog` append-only entity (ToolCribItemId, StepExecutionId FK → StepExecution, EquipmentId FK → Equipment, UnitsConsumed int, OperatorUserId, CheckedOutAt, ReturnedAt, ConditionOnReturn enum Good/Damaged/WornBeyondSpec/Lost, Notes); `ToolRegrindRecord` entity (ToolCribItemId, SentAt, ReturnedAt, VendorId FK → Supplier optional, Cost decimal, QualityCheckResult enum Pass/Fail, MeasuredDimensions JSON); automatic life tracking: when a step execution completes, tool usage is logged and CurrentLifeUnits incremented; automatic reorder: when available stock drops below ReorderPoint, system creates an ActionItem for procurement; tool-to-quality correlation: `ToolCribController` analytics endpoint correlates tool life percentage with NonConformance rates and SPC Cpk values to identify optimal tool change points; `ToolCribController` (item CRUD, checkout/return workflow, usage history, regrind tracking, stock dashboard with low-stock alerts, life analysis with quality correlation, cost-per-part calculation); `ToolCribDashboard.razor` (/tool-crib — stock status grid with life-remaining progress bars, low-stock alerts, tools-in-use board, regrind queue, cost analysis); `ToolDetail.razor` (/tool-crib/{id} — usage history timeline, life curve chart, quality correlation scatter plot, regrind history, cost breakdown); integration with Phase 11 Equipment (tools assigned to equipment/workstation); integration with Phase 19 Warehouse (tool storage locations); integration with Phase 25 Suppliers (tool vendor management); integration with Phase 24 SPC (tool life vs Cpk trending); MCP `get_tool_crib_status` tool (stock levels, tools due for change, low stock alerts, cost summary). **Market fit:** Tool management is the #1 hidden cost in CNC machining, stamping, and injection molding operations. Cutting tool costs alone represent 3-5% of total manufacturing cost, but poor tool management (premature changes, missed changes causing scrap, lost tools, excessive regrind) can push this to 8-12%. Most shops track tools with spreadsheets, sticky notes on tool boards, or not at all. The unique value here is correlating tool life data with quality outcomes — identifying the exact point in a tool's life where quality begins to degrade allows data-driven tool change policies instead of conservative time-based schedules. This feature directly addresses the needs of the CNC/machining customer segment identified in the MVP market analysis and creates a data moat that general-purpose QMS platforms cannot match.

- **Phase 42 — Multi-Site Benchmarking & KPI Federation** — Aggregates and compares quality, production, and compliance KPIs across multiple manufacturing sites (tenants) to enable corporate-level visibility and best-practice identification. `BenchmarkGroup` entity (Code, Name, Description, OwnerTenantId — the corporate/parent tenant, MemberTenantIds list, IsActive); `BenchmarkMetric` entity (GroupId, MetricType enum OEE/FirstPassYield/ScrapRate/CAPACycleTime/NCRRate/OnTimeDelivery/CustomerComplaintRate/AuditFindingRate/TrainingCompliancePct/SupplierAcceptanceRate, SourceTenantId, PeriodStart, PeriodEnd, Value decimal, Rank int computed within group); `BenchmarkDataCollector` BackgroundService (monthly aggregation: queries each member tenant's data via cross-tenant read using PlatformAdmin credentials — OEE from Phase 29, FPY from NC+production data, CAPA cycle time from Phase 27, SPC Cpk averages from Phase 24, training compliance from Phase 16, supplier acceptance from Phase 25; stores as BenchmarkMetric rows; anonymization option: sites can be shown as Site A/B/C instead of real names for competitive sensitivity); `BenchmarkController` (PlatformAdmin-only: group CRUD, member management, metric query with period/type filters, ranking with percentile bands, trend comparison charts, best-practice identification — finds which site has the best metric and surfaces what processes/practices differ); `BenchmarkDashboard.razor` (/platform/benchmarking — PlatformAdmin only, group selector, KPI comparison spider chart across sites, ranking tables with period-over-period delta arrows, best-of-breed site identification per metric, drill-down to site-level contributing factors); `SiteBenchmarkCard.razor` component (embedded in each site's own dashboard showing their rank within the group and gap-to-best for key metrics, visible to site Admin only); integration with all data-generating phases (OEE, SPC, CAPA, NC, Training, Supplier, Conformance); MCP `get_benchmark_status` tool (group ranking summary, top/bottom performers per metric, improvement opportunities). **Market fit:** Enterprise manufacturers with 3+ sites (automotive Tier 1s, contract manufacturers, medical device companies with multiple production facilities) consistently cite cross-site benchmarking as their #1 unmet need. Corporate quality directors need to answer "which plant is best at X, and what can the others learn from them?" Currently this requires manual data collection from each site's separate systems, normalized in Excel — a quarterly exercise that takes weeks. Real-time federated KPIs with automatic ranking eliminate this entirely and create the corporate visibility layer that justifies enterprise-tier pricing. This feature is the key differentiator for the Enterprise subscription plan and the primary driver of multi-site deal expansion (landing at one site and expanding to the corporate level). No lightweight QMS platform offers this — it's currently only available in enterprise PLM/MES suites (Siemens Opcenter, Rockwell Plex) at 10x the price point.

- **Phase 43 — Regulatory Submission & Audit Readiness Package** — Automated generation of compliance evidence packages for regulatory audits (FDA, ISO registrar, customer audits, notified bodies) that assembles cross-system data into a structured, reviewable submission. `AuditReadinessCheck` entity (Standard enum ISO9001/AS9100/IATF16949/ISO13485/FDA21CFR820/FDA21CFR211/ISO14644/Custom, ClauseId FK → StandardsClause optional, CheckType enum DocumentExists/RecordComplete/MetricInRange/TrainingCurrent/CalibrationCurrent/CapaClosed/SupplierApproved/ProcessReleased, Status enum Pass/Fail/Warning/NotApplicable, Evidence text describing what was found, CheckedAt, NextCheckDue); `AuditReadinessReport` entity (Standard, GeneratedAt, GeneratedByUserId, OverallScore decimal 0-100, PassCount/FailCount/WarningCount, ReportBlobPath for PDF); `AuditReadinessEngine` service (runs a comprehensive check battery per standard: for ISO 9001 — verifies QMS documents exist and are Released per clause, management reviews completed within 12 months, CAPA system active with closure rates, training records current for all required competencies, calibration not overdue, internal audit programme active with findings tracked, customer complaints handled, risk register reviewed; for AS9100 — adds product safety requirements, configuration management, FOD prevention, special process validation; for IATF 16949 — adds APQP evidence per part, PPAP completion, MSA for all critical measurements, customer-specific requirements mapped; for FDA 21 CFR 820 — adds design controls, DHR completeness, CAPA effectiveness, complaint trending, supplier controls); `SubmissionPackageBuilder` service (assembles a structured PDF/ZIP package from audit readiness results: cover page with scope and standard, executive summary with overall score, clause-by-clause evidence matrix with pass/fail indicators and links to source records, appendices with supporting documents — QMS docs, training matrix, calibration certs, CAPA summaries, management review minutes); `AuditReadinessController` (run check battery for a standard, get latest report, get historical trend, download package, schedule recurring checks, gap analysis endpoint identifying highest-priority items to fix before next audit); `AuditReadiness.razor` (/audit-readiness — standard selector, overall compliance score gauge, clause-level pass/fail heatmap, gap list sorted by severity, package generation button, trend chart showing readiness score over time); `AuditGapAnalysis.razor` (/audit-readiness/gaps — prioritized remediation list with effort estimates, linked action items for each gap, progress tracking toward target audit date); integration with Phase 17 Conformance (clause coverage feeds readiness checks); integration with Phase 16 Training (competency records feed training checks); integration with Phase 28 Calibration (calibration status feeds equipment checks); integration with Phase 27 CAPA (closure rates and effectiveness feed CAPA checks); integration with Phase 15 Management Review (review completion feeds management checks); integration with Phase 25 Customer Portal (readiness packages shareable via customer portal); MCP `get_audit_readiness` tool (overall score, top gaps, next scheduled check, recommended actions). **Market fit:** Audit preparation is the most time-consuming activity in any quality manager's calendar — a typical ISO 9001 surveillance audit requires 2-4 weeks of preparation assembling evidence from disparate systems. For FDA inspections, the stakes are even higher: an FDA 483 observation can trigger warning letters, consent decrees, and import alerts that cost millions. The #1 fear of every quality manager is "the auditor asks for something we can't produce." An automated readiness engine that continuously monitors compliance status and pre-assembles evidence packages transforms audit preparation from a quarterly fire drill into continuous assurance. No competitor in the lightweight QMS space offers this — Qualio and Greenlight Guru provide document control but not cross-system audit readiness scoring. This feature creates the highest-value selling point for regulated manufacturers: "you will never fail an audit because of incomplete evidence." Companies in medical devices, aerospace, and automotive report spending $50K-$200K annually on audit preparation labor — this feature has a clear, quantifiable ROI that accelerates sales conversations.

- **Phase 44 — Predictive Maintenance & Equipment Health Scoring** — Machine-learning-ready condition-based maintenance system that replaces calendar-based PM schedules with data-driven predictions of equipment failure. `EquipmentHealthScore` entity (EquipmentId FK → Equipment, CalculatedAt, OverallScore decimal 0–100, ComponentScores JSON — per-subsystem breakdown, TrendDirection enum Improving/Stable/Degrading/Critical, PredictedFailureWindow DateOnly? — estimated date range for next unplanned downtime if no intervention, Confidence decimal 0–1); `MaintenancePredictor` entity (EquipmentId, ModelType enum RuleBased/LinearRegression/SurvivalAnalysis, Features JSON — which signals feed the model: vibration trend slope, temperature delta, OEE degradation rate, time-since-last-PM, NC rate on equipment, MTBF history, cycle count, runtime hours, FailureThreshold decimal, IsActive, LastTrainedAt, Accuracy decimal — back-tested prediction accuracy); `EquipmentConditionReading` append-only entity (EquipmentId, ReadingType enum Vibration/Temperature/Current/Pressure/Noise/OilAnalysis/CycleTime, Value decimal, Unit, ReadingAt, Source enum ManualEntry/IoTSensor/PLCFeed — compatible with Phase 38 environmental sensor pattern); rule-based predictor (ships out of box, no ML dependency): scores equipment health using weighted signals — maintenance compliance (overdue PMs reduce score), downtime frequency trend (Phase 11 DowntimeRecord), NC rate correlation (rising NCs on equipment reduce score), age-based degradation curve, condition readings vs baseline; `PredictiveMaintenanceController` (equipment health dashboard with fleet-wide scores, individual equipment health detail with contributing factors, predicted failure timeline, what-if: "if we skip next PM, how does risk change?", maintenance optimization: prioritized PM schedule based on health scores rather than fixed calendar, condition reading ingestion endpoint for IoT/PLC integration); `EquipmentHealthDashboard.razor` (/equipment/health — fleet health heatmap sorted by risk, predicted-failure timeline Gantt chart, health score trending per equipment, maintenance ROI calculator showing avoided downtime); `EquipmentHealthDetail.razor` (/equipment/{id}/health — health score gauge with contributing factors, condition reading time-series, maintenance history impact analysis, similar-equipment comparison); integration with Phase 11 Equipment & Production (downtime records, PM schedules, runtime hours); integration with Phase 24 SPC (equipment-linked process capability degradation as health signal); integration with Phase 8c NonConformance (equipment-correlated NC rate as health signal); integration with Phase 38 Environmental (condition readings share ingestion pattern); MCP `get_equipment_health` tool (fleet health summary, at-risk equipment, predicted failures, recommended actions). **Market fit:** unplanned downtime costs manufacturers an estimated $50B/year globally (Deloitte). Calendar-based preventive maintenance wastes 30-40% of maintenance budgets on unnecessary interventions while still missing 82% of failures (ARC Advisory Group). Predictive maintenance is the #1 Industry 4.0 investment priority for manufacturers (McKinsey, PwC surveys consistently rank it first). Enterprise PdM platforms (Uptake, Senseye, Augury) cost $100K+/year and require dedicated data science teams. Embedding a rule-based equipment health scoring system — using signals the platform already captures (downtime, NCs, PM compliance, OEE) — provides 80% of the value at 5% of the cost. The IoT condition reading ingestion endpoint creates a growth path toward ML-based prediction as customers accumulate data. This is the feature that gets operations and maintenance leadership (VP of Operations, Plant Manager, Maintenance Director) to champion the platform purchase, complementing the quality manager buyer persona that drives initial adoption.

- **Phase 45 — Work Instruction Versioning & Operator Feedback Loop** — Bi-directional work instruction system where operators can flag unclear steps, suggest improvements, and report near-misses directly from the execution interface, creating a continuous improvement feedback loop that drives work instruction quality. `InstructionFeedback` entity (Code auto-generated FB-YYYY-NNN, StepTemplateId FK → StepTemplate, StepExecutionId FK → StepExecution optional — links to specific execution where issue was observed, FeedbackType enum Unclear/Incorrect/SafetyConcern/ImprovementSuggestion/NearMiss/ToolIssue/MaterialIssue, Description, SubmittedByUserId, SubmittedAt, Severity enum Info/Minor/Major/Critical, Status enum New/UnderReview/Accepted/Implemented/Declined, ReviewedByUserId, ReviewedAt, ReviewNotes, LinkedChangeOrderId FK → ChangeOrder optional — accepted feedback can trigger an ECO); `InstructionVersion` entity (StepTemplateId, VersionNumber int, ChangeSummary, ChangedByUserId, ChangedAt, PreviousContent JSON snapshot of prompts/instructions/tolerances before change, ChangeSource enum Planned/FeedbackDriven/AuditFinding/CustomerComplaint/RegulatoryUpdate — tracks why the instruction changed); `NearMissReport` entity (FeedbackId FK → InstructionFeedback, HazardType enum PinchPoint/ChemicalExposure/ElectricalHazard/Ergonomic/SlipTrip/ToolFailure/MaterialDefect, PotentialConsequence enum FirstAid/MedicalTreatment/LostTime/Fatality, PreventiveActionTaken, WitnessUserIds JSON); automatic feedback aggregation: `InstructionFeedbackController` analytics endpoint identifies steps with highest feedback density, correlates feedback-type distribution with NC rates, and ranks steps by improvement priority using weighted score (critical severity × frequency × NC correlation); `InstructionFeedbackController` (submit feedback from execution context, review queue for process engineers, accept/decline workflow, instruction version history, feedback analytics by process/step/type, near-miss reporting and trending); `FeedbackPanel.razor` component (embedded in ExecutionWizard — one-tap feedback button per step during execution, minimal-friction submission with type picker and optional description, near-miss escalation path with hazard classification); `FeedbackQueue.razor` (/feedback — process engineer review queue sorted by severity and frequency, batch accept/decline, direct link to StepTemplate editor for accepted changes, feedback-to-ECO workflow for changes requiring formal change control); `InstructionAnalytics.razor` (/feedback/analytics — step-level feedback heatmap across processes, top-10 most-flagged steps, feedback resolution rate and cycle time, near-miss trending by hazard type, correlation view: steps with high feedback density vs NC rates on those steps); `VersionHistory.razor` component (on StepTemplate editor — shows instruction revision timeline with change source attribution, diff view between versions, feedback that drove each change); integration with Phase 3 Execution (feedback button in ExecutionWizard step UI); integration with Phase 32 ECO (accepted feedback can auto-create draft change order); integration with Phase 8c NonConformance (feedback-flagged steps correlated with NCs); integration with Phase 27 CAPA (near-miss trends feed CAPA initiation); integration with Phase 39 LPA (LPA finding on a step cross-referenced with operator feedback); MCP `get_instruction_feedback` tool (feedback queue summary, top-flagged steps, near-miss stats, improvement velocity). **Market fit:** ISO 9001 clause 7.5.3 requires controlled documented information including work instructions, and clause 10.3 requires continual improvement — but most QMS platforms treat work instructions as static documents managed top-down by engineers. Operators — the people who actually execute the instructions daily — have no structured channel to report that Step 7 is confusing, the torque spec in Step 12 is wrong, or they almost got hurt following Step 3. This feedback gap means work instructions degrade silently until a quality escape or safety incident forces a reactive update. Near-miss reporting is required by OSHA and ISO 45001 but typically collected in separate EHS systems disconnected from the process context where the near-miss occurred. Embedding operator feedback directly in the execution interface — where the context is richest and the friction is lowest — creates the continuous improvement engine that lean manufacturing demands. TWI (Training Within Industry) methodology, experiencing a resurgence in advanced manufacturing, explicitly requires work instruction refinement based on operator input. This feature transforms the platform from a top-down instruction delivery system into a collaborative knowledge management platform, dramatically increasing operator engagement and work instruction accuracy.

- **Phase 46 — Production Traceability & Genealogy Graph** — End-to-end lot/serial genealogy tracking that maps every material input, process step, equipment, operator, and environmental condition to each finished unit — enabling instant containment scoping, regulatory recall management, and customer-facing traceability certificates. `TraceabilityRecord` entity (Code auto-generated TR-YYYY-NNN-SSS, RecordType enum RawMaterial/SubAssembly/FinishedGood/Rework, KindId FK → Kind, LotNumber, SerialNumber optional for serialized products, Quantity decimal, UnitOfMeasure, Status enum Active/Quarantined/Released/Shipped/Recalled, CreatedAt, ReleasedAt, ShippedAt, CustomerName optional, CustomerPO optional); `TraceabilityLink` entity (ParentRecordId FK → TraceabilityRecord, ChildRecordId FK → TraceabilityRecord, LinkType enum MaterialInput/SubAssemblyInput/ReworkInput/SplitFrom/MergedInto, Quantity decimal — how much of the child was consumed into the parent, LinkedAt); `TraceabilityProcessEvidence` entity (TraceabilityRecordId, StepExecutionId FK → StepExecution, EquipmentId FK → Equipment optional, OperatorUserId, ProcessedAt, EnvironmentalReadingIds JSON optional — links to Phase 38 readings during processing, CalibrationRecordIds JSON optional — links to active calibration records for equipment used, MeasurementValues JSON — key measurements recorded during this step); `ContainmentScope` entity (TriggerType enum NonConformance/CustomerComplaint/SupplierNotification/InternalAudit/RegulatoryRecall, TriggerEntityId, AffectedTraceabilityRecordIds JSON, ScopeDirection enum Forward/Backward/Both, ForwardReachCount int — how many downstream records affected, BackwardReachCount int, ScopedAt, ScopedByUserId, ContainmentAction enum None/HoldForInspection/Quarantine/Rework/Scrap/CustomerNotification/Recall, ContainmentStatus enum Pending/InProgress/Complete); genealogy graph engine: given any TraceabilityRecord, recursively traverse TraceabilityLinks to build complete forward genealogy (where did this material end up?) and backward genealogy (what went into this product?); containment analysis: given a defective lot/material, forward-trace to identify all affected finished goods and their customers; given a customer complaint, backward-trace to identify all input materials and process conditions; `TraceabilityController` (record CRUD with lot/serial management, link materials to production records, forward/backward genealogy query with depth limit, containment scoping endpoint — given a trigger, computes full blast radius with affected record counts, traceability certificate generation endpoint — assembles complete production history for a finished good including all input materials with lot numbers, process steps with timestamps and operators, equipment with calibration status, measurements, and environmental conditions); `TraceabilityDashboard.razor` (/traceability — lot/serial search with forward/backward trace, interactive genealogy graph visualization using force-directed layout, containment simulator — "if lot X is defective, what is affected?", active containment actions status board); `GenealogyGraph.razor` component (D3.js or Cytoscape.js interactive directed graph: nodes = TraceabilityRecords color-coded by type, edges = TraceabilityLinks labeled by quantity, click node to expand genealogy, highlight containment scope in red, show process evidence on hover); `TraceabilityCertificate.razor` (/traceability/{id}/certificate — printable/PDF production history document: bill of materials with lot numbers, process routing with timestamps and operator names, equipment list with calibration dates, inspection results, environmental conditions — suitable for customer or regulatory submission); `ContainmentWizard.razor` (/traceability/containment — step-by-step containment workflow: select trigger, system computes scope, review affected records, assign containment action, track through completion); integration with Phase 19 Warehouse (material receipts create raw material TraceabilityRecords, lot numbers link to InventoryItems); integration with Phase 3 Execution (step executions auto-create TraceabilityProcessEvidence records); integration with Phase 11 Production (job completion creates finished good TraceabilityRecord linked to consumed materials); integration with Phase 8c NonConformance (NC on a lot triggers containment scoping); integration with Phase 34 Customer Complaint (complaint triggers backward trace from shipped lot); integration with Phase 28 Calibration (calibration records linked as evidence); integration with Phase 38 Environmental (readings linked as evidence); integration with Phase 30 EBR (batch record incorporates genealogy data as release evidence); MCP `get_traceability` tool (lot/serial lookup, genealogy summary, containment scope preview, active containments). **Market fit:** lot traceability is a non-negotiable regulatory requirement across every regulated manufacturing sector — FDA 21 CFR 820.184 (medical device DHR), 21 CFR 211.188 (pharma batch records), EU MDR Article 25 (UDI traceability), IATF 16949 clause 8.5.2 (identification and traceability), AS9100 clause 8.5.2.1 (product acceptance records). When a recall occurs, the speed of containment directly determines financial impact — each hour of delay in scoping an automotive recall can mean thousands of additional vehicles affected. The 2024 FDA Food Safety Modernization Act (FSMA) Rule 204 now requires one-step-forward, one-step-backward traceability within 24 hours for food manufacturers. Most manufacturers maintain traceability in paper travelers, disconnected spreadsheets, or ERP systems that only track materials (not process conditions, operators, or equipment). Full genealogy — connecting materials through process steps to finished goods with all associated quality evidence — is the holy grail of manufacturing digitization and the feature that separates a process management tool from a true Manufacturing Execution System (MES). This feature addresses the single most common question manufacturers face during audits and customer visits: "show me everything that went into this product and prove it was made correctly." Companies with product liability exposure (medical devices, aerospace, automotive, food) consistently rank traceability as their #1 software requirement and are willing to pay premium pricing for it.

- **Phase 47 — Warranty Claims & Return Material Authorization (RMA)** — End-to-end warranty claim management linking field failures back to production data for root cause analysis and warranty cost attribution. `WarrantyClaim` entity (Code auto-generated WC-YYYY-NNN, CustomerName, CustomerPO, ProductKindId FK → Kind, SerialNumber optional, LotNumber optional, FailureDate, ClaimDate, FailureMode enum EarlyLife/RandomFailure/WearOut/EnvironmentalDamage/Misuse/Unknown, FailureDescription, QuantityAffected, Status enum Received/UnderReview/Approved/Denied/CreditIssued/ReplacementSent/Closed, WarrantyType enum Standard/Extended/Goodwill/Recall, ApprovedByUserId, ClaimCost decimal, CreditAmount decimal, LinkedComplaintId FK → CustomerComplaint optional, LinkedNonConformanceId FK → NonConformance optional, LinkedTraceabilityRecordId FK → TraceabilityRecord optional); `RmaRequest` entity (WarrantyClaimId, RmaNumber auto-generated RMA-YYYY-NNN, ShippingInstructions, ReturnAddress, CarrierTracking, ReceivedAt, DispositionType enum Repair/Replace/Refund/NoFaultFound/Scrap, InspectionFindings, InspectedByUserId); `WarrantyPolicy` entity (KindId FK → Kind, WarrantyDuration months, WarrantyType, CoverageTerms, ExclusionCriteria); failure mode trending: `WarrantyController` analytics endpoint correlates field failure modes with production batches, identifies serial/lot containment scope, computes warranty cost per product/lot/time period, MTBF/MTTF estimation from failure date vs production date; `WarrantyController` (claims CRUD with customer/product/status filters, approve/deny with reason, RMA issuance with auto-generated number, receive/inspect returned material, link to complaint/NC/traceability, cost attribution, dashboard with cost trending and failure mode Pareto, MTBF estimation); `WarrantyDashboard.razor` (/warranty — KPI cards: open claims/total cost this quarter/approval rate/avg resolution days, failure mode Pareto, cost trending by month, product reliability table with MTBF estimates, containment alerts when multiple claims affect same lot); `WarrantyClaimDetail.razor` (/warranty/{id} — claim timeline, RMA tracking, inspection results, linked production data via traceability, cost breakdown); integration with Phase 34 Customer Complaint (complaint can escalate to warranty claim); integration with Phase 46 Traceability (lot/serial lookup to find production conditions for failed unit); integration with Phase 35 Cost of Quality (warranty costs feed external failure category); integration with Phase 25 Supplier (supplier-caused field failures escalate to supplier); MCP `get_warranty_status` tool (open claims, cost summary, failure mode trending, reliability metrics). **Market fit:** Warranty cost is the single largest hidden quality cost for product manufacturers — automotive suppliers report 1-3% of revenue consumed by warranty, medical device companies face FDA MDR (Medical Device Reporting) requirements for field failures, and consumer electronics manufacturers need rapid field-to-factory feedback loops. Currently warranty management lives in ERP systems (SAP QM, Oracle) or CRM tools completely disconnected from production quality data — meaning when a field failure occurs, the connection between the failed unit and its production conditions requires manual investigation. Linking warranty claims directly to production traceability (which lot, which operator, which equipment, which supplier material) enables instant root cause narrowing that can reduce investigation time from weeks to hours. The MTBF estimation from warranty data feeds product reliability engineering — a critical input for aerospace (AS9100 8.1.1 product safety) and medical (ISO 14971 risk management) requirements. This feature closes the loop between field performance and factory quality, making the platform the single system of record for the entire product quality lifecycle.

- **Phase 48 — Shift Handover & Production Logbook** — Structured shift handover documentation that captures production status, open issues, safety concerns, and outstanding actions at each shift change — replacing paper logbooks and verbal handovers that lose critical context. `ShiftHandover` entity (Code auto-generated SH-YYYY-NNN, ShiftDefinitionId FK → ShiftDefinition, HandoverDate, OutgoingOperatorId FK → User, IncomingOperatorId FK → User optional — null until acknowledged, Status enum Draft/Submitted/Acknowledged/Disputed, OverallStatus enum Normal/Caution/Critical, ProductionSummary text — what was produced, QuantityProduced int, QuantityTarget int, ScheduleAdherence computed produced/target percentage); `HandoverItem` entity (ShiftHandoverId, Category enum ProductionIssue/QualityAlert/SafetyConcern/EquipmentIssue/MaterialShortage/PendingAction/Information, Priority enum Low/Medium/High/Critical, Description, ActionRequired bool, ActionStatus enum Open/InProgress/Resolved, AssignedToUserId optional, ResolvedAt, LinkedEntityType enum Job/Equipment/NonConformance/MaintenanceTask/CustomerComplaint/ChangeOrder optional, LinkedEntityId optional); `ShiftLog` append-only entity (ShiftDefinitionId, OperatorUserId, LogTime, Category enum Production/Quality/Maintenance/Safety/General, Message, Severity enum Info/Warning/Alert); auto-population: when creating a shift handover, system pre-fills data from the current shift — open jobs and their status from Phase 5, active equipment alarms from Phase 11, open NCs from Phase 8c, overdue maintenance from Phase 11, active Andon events from Phase 33 (when built), pending action items from Phase 15; incoming operator acknowledgment: the receiving operator must review all items and either acknowledge (accepting responsibility for open items) or dispute (flagging items that need clarification before accepting); handover chain: system maintains a continuous chain of handovers per shift schedule, preventing gaps; `ShiftHandoverController` (CRUD, submit handover, acknowledge/dispute by incoming operator, shift log entries, handover history per shift schedule, dashboard with handover compliance rate and open-item aging, unacknowledged handovers alert); `ShiftHandover.razor` (/shift-handover — mobile-optimized creation form pre-filled from system data, item categorization with quick-add, production count entry, overall status assessment, submit to incoming shift); `HandoverReview.razor` (/shift-handover/{id}/review — incoming operator view: review all items, acknowledge or dispute, add notes); `ShiftLogbook.razor` (/shift-log — real-time shift logbook with chronological entries, category filter, severity filter, add entry form — replaces paper logbook); `HandoverDashboard.razor` (/shift-handover/dashboard — compliance rate gauge, open item trending, recurring issues identification, shift-to-shift performance comparison); integration with Phase 29 OEE (shift OEE pre-filled in handover); integration with Phase 11 Production (WIP status, equipment alarms); integration with Phase 33 Andon (active Andon events included); integration with Phase 15 Action Items (open actions flagged in handover); MCP `get_shift_handover_status` tool (latest handover per shift, open items, compliance rate, recurring issues). **Market fit:** shift handover failure is the #1 cited root cause of production incidents in continuous manufacturing (chemical, pharma, food, metals) and a top-5 root cause in discrete manufacturing. The UK Health & Safety Executive found that 20% of manufacturing safety incidents are linked to inadequate shift handover communication. Regulatory frameworks explicitly require documented shift handover: FDA 21 CFR 211.192 (pharma production record review), OSHA PSM 1910.119(f) (process safety management), and nuclear NRC 10 CFR 26 (fatigue management). Currently manufacturers use paper logbooks that are never reviewed, verbal handovers that lose 60-80% of information (NASA studies on shift handover information retention), or WhatsApp groups that have no audit trail. A structured, system-integrated shift handover that pre-populates from live production data, requires acknowledgment, and creates a searchable audit trail directly addresses a safety-critical communication gap. The shift logbook feature also provides the raw material for AI-driven analysis of recurring issues across shifts — identifying patterns (e.g., "Line 3 always has quality issues on night shift after maintenance window") that no human can detect from paper logs. This is the feature that production supervisors and plant managers will immediately recognize as solving their daily pain.

- **Phase 49 — Advanced Reporting Engine & Custom Dashboard Builder** — Self-service analytics platform enabling quality engineers and managers to create custom reports, dashboards, and scheduled email digests without developer intervention — replacing the current fixed-layout report pages with a flexible, user-configurable reporting layer. `ReportDefinition` entity (Code, Name, Description, OwnerId FK → User, Visibility enum Private/Team/Organization, ReportType enum Tabular/Chart/KpiCard/Composite, DataSource enum Jobs/StepExecutions/NonConformances/ActionItems/CustomerComplaints/CapaRecords/ChangeOrders/CalibrationRecords/SpcDataPoints/SupplierEvaluations/Inventory/OEE/GageStudies/TrainingRecords, Filters JSON — array of {field, operator, value} conditions, Columns JSON — array of {field, label, aggregation, format}, GroupBy JSON — optional array of grouping fields, SortBy JSON, ChartConfig JSON — chart type/axes/series/colors for visualization, DateRange enum Today/ThisWeek/ThisMonth/ThisQuarter/ThisYear/Last30Days/Last90Days/Custom/Rolling, IsScheduled bool, ScheduleCron string optional, ScheduleRecipients JSON optional — email addresses for digest delivery); `ReportSnapshot` entity (ReportDefinitionId, GeneratedAt, DataJson, RowCount int — stores periodic snapshots for trend-over-time comparison and scheduled email content); `DashboardLayout` entity (Code, Name, OwnerId, Visibility, LayoutConfig JSON — grid-based widget placement with responsive breakpoints, Widgets JSON array of {reportDefinitionId, position, size, refreshInterval}); `ReportTemplate` seed entities — 15+ pre-built report templates covering common manufacturing analytics: CAPA aging report, NC Pareto by product, supplier scorecard trending, training compliance matrix, calibration recall schedule, SPC Cpk trending by process, OEE by equipment, customer complaint response time, change order cycle time, action item overdue aging, shift production summary, first pass yield by process, scrap rate trending, warranty cost by product, audit finding closure rate; `ReportEngine` service (translates ReportDefinition filters/columns/groupings into dynamic LINQ queries against the appropriate DbSet, handles aggregation SUM/AVG/COUNT/MIN/MAX/DISTINCT, handles date truncation Day/Week/Month/Quarter for time-series, returns structured DataTable result); `ChartRenderer` service (translates ChartConfig + data into Chart.js-compatible JSON for frontend rendering — bar/line/pie/donut/scatter/heatmap/stacked-bar/waterfall); `ReportSchedulerService` BackgroundService (evaluates scheduled reports on cron, generates snapshot, renders HTML email with inline charts, delivers via configurable SMTP/SendGrid); `ReportsController` (report definition CRUD, execute report returning paginated data, export to CSV/Excel/PDF, snapshot history, dashboard layout CRUD, template library endpoint); `ReportBuilder.razor` (/reports/builder — drag-and-drop report designer: data source selector, available fields list, filter builder with type-aware operators, column selector with aggregation picker, live preview pane, chart type selector with axis mapping, save/share); `DashboardBuilder.razor` (/dashboards/builder — grid layout editor, add widget from saved reports, resize/reposition, configure refresh interval per widget, responsive breakpoint preview); `CustomDashboard.razor` (/dashboards/{code} — rendered dashboard with live-refreshing widgets, full-screen mode for wall displays, auto-cycle between multiple dashboards); `ReportViewer.razor` (/reports/{code} — rendered report with filter controls, pagination, export buttons, chart/table toggle, drill-down on chart elements); `ScheduledReports.razor` (/reports/scheduled — manage scheduled deliveries, preview email format, recipient management, delivery history); integration with all data-producing phases (every entity with a DbSet becomes a queryable data source); integration with Phase 33 Shop Floor Display (custom dashboards can serve as display layouts); integration with Phase 42 Benchmarking (cross-site comparison reports); MCP `run_report` tool (execute a named report with optional filter overrides, return structured data). **Market fit:** reporting is consistently the #1 feature request in QMS/MES platforms post-deployment — every organization has unique KPIs, management reporting requirements, and customer-specific metrics that fixed-layout dashboards cannot serve. Quality managers spend 4-8 hours/week manually assembling data from multiple screens into PowerPoint presentations for management review (ISO 9001 clause 9.3 input requirements). The current Process Manager has excellent data capture but limited data extraction — everything is in fixed-layout pages designed for operational use, not analytical reporting. A self-service report builder with scheduled email delivery eliminates the "data is in the system but I can't get it out in the format I need" complaint that drives users back to Excel. Competitors (ETQ Reliance, MasterControl, Qualio) all offer custom reporting as a premium/enterprise feature — it's the #1 reason companies upgrade from starter to professional plans. The pre-built template library provides immediate value (users can start with templates and customize) while the drag-and-drop builder enables power users to create anything. Scheduled email digests solve the "management doesn't log into the system" problem by pushing the right data to the right people at the right cadence — this single feature can increase platform stickiness 3x because leadership becomes dependent on the automated reports even if they never touch the UI directly.

- **Phase 50 — Strategy Management: Balanced Scorecard** — Kaplan/Norton Balanced Scorecard builder under a new **Strategy** nav section. Scorecards with customizable perspectives (Financial / Customer / Internal Business Process / Learning & Growth seeded by default), strategic objectives with OrgUnit owners, measures with baseline/target/RAG thresholds, strategy-map cause-and-effect links between objectives, and initiatives spawned as `ActionItem`s. Measures resolve live from existing modules — process yield from PortTransaction grades, step maturity from `MaturityScoringService`, SPC Cpk, action close rate, training compliance, OEE, cost of quality — so a linked process's yield expresses the maturity of the objective it serves. Full design in the **Phase 50** section above. **Market fit:** every ISO 9001 organization must demonstrate strategic planning and management review with measurable quality objectives (clauses 5.2, 6.2, 9.3); today this lives in PowerPoint decks disconnected from operational data. A scorecard that auto-feeds from live execution data closes the strategy-to-shop-floor loop competitors don't offer.
