...
694 integration tests across all phases, all passing. Tests run against an in-memory SQLite database spun up per test run. Test files cover all controllers including Analytics, Alerts, Reports, Document Library filters, SPC, Workstations, and API Keys.
...
- **Phase 27 — Corrective & Preventive Action (CAPA) Workflow** — Structured 8D/CAPA process built on the existing ActionItem and RCA infrastructure. `CapaRecord` entity (Code auto-generated CAPA-YYYY-NNN, Type enum Corrective/Preventive, SourceType enum CustomerComplaint/InternalAudit/SupplierIssue/SpcOutOfControl/NonConformance/ManagementReview, SourceEntityId, ProblemStatement, ContainmentAction, RootCauseAnalysisId FK → IshikawaDiagram or FiveWhysAnalysis, PermanentCorrectiveAction, PreventiveAction, VerificationMethod, VerificationDueDate, VerifiedAt, EffectivenessReviewDate, EffectivenessVerifiedAt, Status enum Open/Containment/RCA/Implementation/Verification/EffectivenessReview/Closed, OwnerId FK → User, TeamMemberIds JSON); `CapaStep` immutable audit log (CapaId, StepType enum, CompletedBy, CompletedAt, Notes, AttachmentFileName); auto-CAPA creation triggers (SPC Rule 1 violation on critical characteristic → Corrective CAPA, recurring NCR on same failure mode → Preventive CAPA, major audit finding → Corrective CAPA); `CapaController` (CRUD, lifecycle transitions, team management, linked action items, verification and effectiveness endpoints); `CapaList.razor` (/capa — paginated table with status/type/source filter, overdue badges, create modal); `CapaDetail.razor` (/capa/{id} — 8D-style step timeline, containment/RCA/corrective/preventive/verification sections, linked NCRs and action items, effectiveness review scheduling); `CapaDashboard.razor` (/capa/dashboard — KPI cards: open/overdue/avg-days-to-close, source breakdown, effectiveness rate); integration with Phase 24 SPC (OOC events trigger CAPA); integration with Phase 17 (audit findings link to CAPA); MCP `get_capa_status` tool; MCP `create_capa` write tool. **Market fit:** formal CAPA management is required by ISO 9001 (clause 10.2), AS9100 (clause 10.2 + 10.2.1), FDA 21 CFR 820.90 (medical devices), and IATF 16949 (clause 10.2.3). Most QMS platforms offer basic CAPA tracking but lack the integration with SPC triggers, RCA tools, and audit findings that turns CAPA from a paperwork exercise into a closed-loop improvement system. This is the single most-audited quality process.
