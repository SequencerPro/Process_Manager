...
734 integration tests across all phases. Tests run against an in-memory SQLite database spun up per test run. Test files cover all controllers including Analytics, Alerts, Reports, Document Library filters, SPC, Workstations, API Keys, Suppliers, and CAPA.
...
#### Partially built
*(none at this time — Phase 27 CAPA moved to complete)*
...
- **Phase 26 — Measurement System Analysis (MSA/GR&R)** — Gage Repeatability & Reproducibility studies embedded in the process model. `GageStudy` entity (Name, StudyType enum GRR_CrossedANOVA/GRR_Range/Linearity/Bias/Stability, GageId FK → Equipment, ProcessId, CharacteristicName, Tolerance, NumberOfParts, NumberOfOperators, NumberOfTrials, Status enum Draft/InProgress/Complete); `GageStudyMeasurement` entity (StudyId, PartNumber, OperatorId, TrialNumber, MeasuredValue); `GrrCalculationService` (Range method: R̄ by operator, R̄ overall, d₂ lookup, EV/AV/GRR/PV/TV decomposition, %GRR of tolerance and of total variation, number of distinct categories ndc; ANOVA method: two-way ANOVA with interaction, variance component extraction, confidence intervals); `GageStudyController` (CRUD, add measurements in bulk, calculate results, accept/reject decision with %GRR thresholds configurable per study); `GageStudyList.razor` (/gage-studies — studies table with %GRR badge color-coded green<10/amber<30/red); `GageStudyDetail.razor` (/gage-studies/{id} — measurement entry grid, results panel with EV/AV/GRR/PV/TV breakdown, by-operator box plot, part-by-operator interaction plot, acceptance decision); integration with Control Plan (link GR&R study to measurement method); MCP `get_msa_status` tool. **Market fit:** MSA is a mandatory component of IATF 16949 (clause 7.1.5.1.1) and PPAP (Element 6). Most manufacturers run GR&R studies in Minitab or Excel — embedding them means the measurement system qualification lives alongside the control plan and SPC data, creating a complete measurement traceability chain.
- **Phase 27 — CAPA Workflow** ✅ *(implemented — see completed section above)*
- **Phase 28 — Calibration Management** — Equipment and gage calibration lifecycle tracking with recall scheduling. `CalibrationRecord` entity (EquipmentId FK → Equipment, CalibrationType enum Internal/External/Vendor, CalibrationDate, NextDueDate, CertificateNumber, CertificateFileName, Result enum Pass/Fail/Limited, PerformedBy, StandardsUsed, TemperatureHumidity, AsFoundReading, AsLeftReading, Uncertainty, Notes); `CalibrationSchedule` entity (EquipmentId, IntervalDays, IntervalAdjustmentMethod enum Fixed/ReliabilityBased, LastResult, ConsecutivePassCount, MaxIntervalDays, MinIntervalDays); reliability-based interval adjustment: consecutive passes extend interval by configurable %, any fail resets to minimum; `CalibrationRecallService` (BackgroundService polling daily, identifies equipment past NextDueDate, creates ActionItem for calibration team, fires `calibration.due` and `calibration.overdue` webhook events, optionally quarantines equipment with auto-downtime record); `CalibrationController` (CRUD for records and schedules, recall dashboard with due/overdue/expired lists, certificate upload/download, calibration history timeline per equipment); `CalibrationList.razor` (/calibration — due/overdue/all tabs, equipment search, recall badge counts); `CalibrationDetail.razor` integrated into `EquipmentDetail.razor` as a new tab (calibration history, certificate viewer, next-due countdown, schedule configuration); integration with Phase 24 SPC (measurement instrument traceability — SPC chart references calibrated gage); integration with Phase 11 Equipment (calibration status badge on equipment list); MCP `get_calibration_status` tool (due/overdue/expired counts, next recall dates); MCP `list_calibration_recalls` tool. **Market fit:** calibration management is required by ISO 9001 (clause 7.1.5.2), AS9100 (clause 7.1.5.2), and IATF 16949 (clause 7.1.5.2.1). FDA 21 CFR 820.72 mandates it for medical devices. Most manufacturers use standalone calibration tracking systems (GageSuite, Fluke Calibration, ProCalV5) disconnected from both their equipment registry and their quality system — this creates audit risk when a gage used in SPC is found to have lapsed calibration. Embedding calibration into the equipment-SPC-quality chain closes this gap.
...
- **Phase 29 — OEE (Overall Equipment Effectiveness) Dashboard** — Real-time OEE tracking per workstation and equipment asset, computed from existing execution and downtime data. `OeeSnapshot` transient DTO (EquipmentId, ShiftDate, ShiftNumber, AvailabilityPct, PerformancePct, QualityPct, OeePct); `ShiftDefinition` entity (Code, Name, StartTime, EndTime, IsActive — defines shift boundaries for time-bucketing); `OeeCalculationService` computes from existing data: **Availability** = (ShiftDuration − PlannedDowntime − UnplannedDowntime) / ShiftDuration (from `DowntimeRecord` entities), **Performance** = (IdealCycleTime × TotalPiecesProduced) / RunTime (from `StepExecution` durations and `PortTransaction` counts), **Quality** = GoodPieces / TotalPieces (from `NonConformance` scrap/rework counts subtracted from total output); `OeeDashboardController` (GET `/api/oee/dashboard` with date range and equipment filters, GET `/api/oee/trend` for time-series OEE by shift, GET `/api/oee/losses` for Pareto of loss categories); `OeeDashboard.razor` (/oee — live OEE gauge per equipment, 7-day trend sparklines, Pareto chart of top loss categories, drill-down to shift-level breakdown); `ShiftList.razor` (/admin/shifts — shift definition CRUD); integration with Phase 11 equipment/downtime (DowntimeRecord feeds availability calculation); integration with Phase 8c NonConformance (scrap/rework feeds quality calculation); MCP `get_oee_status` tool. **Market fit:** OEE is the universal manufacturing KPI — every lean manufacturing initiative, TPM programme, and continuous improvement effort tracks it. Most manufacturers compute OEE manually in spreadsheets or use dedicated OEE hardware systems (Vorne XL, Redzone) disconnected from their quality system. Embedding OEE into the process/quality platform means availability, performance, and quality losses are traceable to specific root causes, NCRs, and CAPAs — closing the loop between "what happened" and "what do we do about it." This is the #1 feature request from manufacturing operations teams evaluating QMS/MES platforms.
