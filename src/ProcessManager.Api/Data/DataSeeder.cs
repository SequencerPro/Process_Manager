...
public static class DataSeeder
{
    ...
    public static async Task SeedQmsDocumentsAsync(ProcessManagerDbContext db)
    {
        ...
        // ── QMS-001 receives full rich content blocks (ISO 9001:2015 clause 4.3) ─
        {
            ...
            {
                Id = Guid.NewGuid(), CreatedAt = qms001.CreatedAt, UpdatedAt = qms001.CreatedAt,
                ProcessId = qms001.Id, StepTemplateId = stDocSect.Id, Sequence = 1,
                NameOverride = "Purpose and Scope",
                DescriptionOverride = "Defines the boundaries and applicability of the QMS per ISO 9001:2015 clause 4.3."
            };
            ps1.Contents.Add(Blk(ps1, 0,
                "1.1  Purpose\n\n" +
                "This document defines the scope of the organisation's Quality Management System (QMS) " +
                "in accordance with ISO 9001:2015 clause 4.3. The scope statement is a mandatory piece of " +
                "documented information. It must clearly state which products and services are covered, which " +
                "sites and functions are included, and which — if any — requirements of ISO 9001:2015 have " +
                "been excluded together with the justification for those exclusions."));
            ps1.Contents.Add(Blk(ps1, 1,
                "1.2  Scope of the QMS\n\n" +
                "The QMS covers all activities undertaken by the organisation in connection with the design, " +
                "manufacture, test, inspection, and delivery of its products and services. This includes:\n\n" +
                "  • All manufacturing processes and associated support functions (engineering, planning, quality assurance)\n" +
                "  • All controlled documentation maintained under the document control procedure (QMS-006)\n" +
                "  • All suppliers and external providers whose outputs are incorporated into, or directly affect, " +
                "the conformity of the organisation's products and services\n" +
                "  • All physical sites at which any of the above activities are performed"));
            ps1.Contents.Add(Blk(ps1, 2,
                "1.3  Exclusions\n\n" +
                "Where a requirement of ISO 9001:2015 is determined to be not applicable to the nature or " +
                "scope of the organisation's activities, it shall be identified here with written justification " +
                "demonstrating that the exclusion does not affect the organisation's ability or responsibility " +
                "to ensure the conformity of its products and services.\n\n" +
                "No exclusions are applicable at the time of this revision. Any future exclusions require a " +
                "formal revision with top-management sign-off and a re-certification assessment by the " +
                "certification body."));
            ps1.Contents.Add(Blk(ps1, 3,
                "1.4  Relationship to organisational context\n\n" +
                "The scope boundary is informed by the analyses required under clauses 4.1 (understanding " +
                "the organisation and its context) and 4.2 (understanding the needs and expectations of " +
                "interested parties). The outputs of those analyses — the organisational context review and " +
                "the interested-parties register — are maintained alongside the risk register (QMS-005) and " +
                "are reviewed at each management review (QMS-019) to confirm that the scope remains appropriate."));
            qms001.ProcessSteps.Add(ps1);

            var ps2 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms001.CreatedAt, UpdatedAt = qms001.CreatedAt,
                ProcessId = qms001.Id, StepTemplateId = stDocSect.Id, Sequence = 2,
                NameOverride = "Responsibilities",
                DescriptionOverride = "Accountability for defining, maintaining, and communicating the QMS scope."
            };
            ps2.Contents.Add(Blk(ps2, 0,
                "Top Management\n\n" +
                "Accountable for defining the QMS scope and for approving all revisions before release. " +
                "The organisation's Chief Executive or designated management representative signs and dates " +
                "the approved scope statement. No revision to the scope takes effect without this approval."));
            ps2.Contents.Add(Blk(ps2, 1,
                "Quality Manager\n\n" +
                "Responsible for:\n" +
                "  • Initiating, drafting, and reviewing the scope statement whenever organisational context changes\n" +
                "  • Conducting the interested-parties analysis in advance of each scope review\n" +
                "  • Submitting the scope document for approval through the standard approval workflow\n" +
                "  • Communicating the approved scope to all relevant personnel and making it available to interested parties on request\n" +
                "  • Maintaining this document under the document control procedure (QMS-006)\n" +
                "  • Ensuring alignment between the scope statement and the organisation's ISO 9001 certification status"));
            ps2.Contents.Add(Blk(ps2, 2,
                "Process Owners\n\n" +
                "Responsible for:\n" +
                "  • Reviewing and confirming that their functions and processes are correctly represented in the scope\n" +
                "  • Notifying the Quality Manager of any changes to their activities, locations, or services that may affect the scope boundary\n" +
                "  • Supporting audit queries relating to the scope of their process area"));
            ps2.Contents.Add(Blk(ps2, 3,
                "All Personnel\n\n" +
                "All employees should be aware of the QMS scope and understand which of their activities fall " +
                "within its boundaries. The scope statement is communicated at induction and is permanently " +
                "available through the Document Library."));
            qms001.ProcessSteps.Add(ps2);

            var ps3 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms001.CreatedAt, UpdatedAt = qms001.CreatedAt,
                ProcessId = qms001.Id, StepTemplateId = stDocSect.Id, Sequence = 3,
                NameOverride = "Procedure",
                DescriptionOverride = "Steps for establishing, updating, approving, and communicating the QMS scope."
            };
            ps3.Contents.Add(Blk(ps3, 0,
                "Step 1 — Analyse organisational context (clause 4.1)\n\n" +
                "The Quality Manager, working with senior management, reviews the internal and external issues " +
                "relevant to the organisation's purpose and strategic direction. Consider: the marketplace and " +
                "competitive environment; technological developments; regulatory and statutory requirements; " +
                "economic, social and environmental conditions; the organisation's culture, knowledge and " +
                "performance; and internal resource availability and capability. Record findings in the " +
                "organisational context analysis document."));
            ps3.Contents.Add(Blk(ps3, 1,
                "Step 2 — Identify and review interested parties (clause 4.2)\n\n" +
                "Compile or update the interested-parties register, listing all parties whose needs and " +
                "expectations are relevant to the QMS. For each party, document their applicable requirements, " +
                "whether those requirements fall within the QMS boundary, and how the organisation monitors or " +
                "engages with them. Interested parties typically include: customers, regulatory and statutory " +
                "bodies, the certification body, suppliers and external providers, employees and their " +
                "representatives, shareholders or owners, and the local community."));
            ps3.Contents.Add(Blk(ps3, 2,
                "Step 3 — Define scope boundaries (clause 4.3)\n\n" +
                "Using the outputs of steps 1 and 2, draft a scope statement that:\n" +
                "  • Identifies the products and services included in the QMS\n" +
                "  • Names the physical sites, functions, and divisions included\n" +
                "  • States any ISO 9001:2015 requirements excluded, with documented justification for each\n" +
                "  • References the interested-parties analysis and risk register that underpin the boundary decisions\n\n" +
                "The scope statement must be sufficiently clear that an external auditor can determine what is " +
                "and is not covered without seeking clarification."));
            ps3.Contents.Add(Blk(ps3, 3,
                "Step 4 — Obtain approval\n\n" +
                "Submit the draft scope document for approval via the standard document control approval workflow " +
                "(see QMS-006 and the 'Standard Document Approval' process template). All approvers must review " +
                "and confirm that the scope correctly represents the organisation's activities and that any claimed " +
                "exclusions are justified. The approval records are retained as part of the document history and " +
                "are available to auditors on request."));
            ps3.Contents.Add(Blk(ps3, 4,
                "Step 5 — Communicate the approved scope\n\n" +
                "Once released, the Quality Manager communicates the current scope:\n" +
                "  • To all employees via the Document Library and induction training (TRN-SYS-001)\n" +
                "  • To customers as part of the quality plan, on request, or where contractually required\n" +
                "  • To the certification body at audit or as a notification when a significant scope change occurs\n\n" +
                "The current approved revision supersedes all previous scope statement versions. Superseded " +
                "versions are retained in the document history but are marked as Superseded and must not be used."));
            ps3.Contents.Add(Blk(ps3, 5,
                "Step 6 — Maintain and review\n\n" +
                "The scope shall be reviewed:\n" +
                "  • Annually as a standing item on the management review agenda (QMS-019)\n" +
                "  • Whenever a significant internal change occurs (new product line, site acquisition or closure, " +
                "outsourcing of a major function, significant change to the product portfolio)\n" +
                "  • Whenever an external change materially affects the organisation's context or interested-party requirements\n" +
                "  • Prior to certification surveillance or re-certification audits\n\n" +
                "If the review concludes no change is required, record the outcome in the management review " +
                "minutes. No new revision is needed unless the scope statement wording changes."));
            qms001.ProcessSteps.Add(ps3);

            var ps4 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms001.CreatedAt, UpdatedAt = qms001.CreatedAt,
                ProcessId = qms001.Id, StepTemplateId = stDocSect.Id, Sequence = 4,
                NameOverride = "Records and Documented Information",
                DescriptionOverride = "Documented information required by or associated with this procedure."
            };
            ps4.Contents.Add(Blk(ps4, 0,
                "The following documented information shall be maintained or retained in connection with this procedure:\n\n" +
                "  • QMS-001 (this document) — the current approved scope statement; maintained as a Released controlled document\n" +
                "  • Interested-parties register — identifies all relevant interested parties and their applicable requirements; " +
                "maintained by the Quality Manager; reviewed at minimum annually\n" +
                "  • Organisational context analysis — records of internal and external issues analysis; reviewed at management review\n" +
                "  • Management review minutes — must include a confirmed review of the QMS scope; see QMS-019\n" +
                "  • Document approval records — approval history for each revision of QMS-001, retained as part of the " +
                "Process Manager document history\n" +
                "  • Exclusion justification records — where applicable, written justification for any ISO 9001:2015 " +
                "requirements excluded from scope; retained for as long as the exclusion applies"));
            ps4.Contents.Add(Blk(ps4, 1,
                "Retention periods\n\n" +
                "All records associated with this procedure shall be retained for a minimum of three years from " +
                "the date of the relevant scope revision, or for the full duration of the associated ISO 9001 " +
                "certification cycle, whichever is the longer. Electronic records maintained in Process Manager " +
                "are subject to the system data retention and backup policy."));
            qms001.ProcessSteps.Add(ps4);
        }

        // ── QMS-002 — full rich content blocks (ISO 9001:2015 clause 5.2) ────
        {
            static ProcessStepContent Blk(ProcessStep ps, int order, string body) => new()
            {
                Id = Guid.NewGuid(), CreatedAt = ps.CreatedAt, UpdatedAt = ps.CreatedAt,
            ...
            var ps1 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms002.CreatedAt, UpdatedAt = qms002.CreatedAt,
                ProcessId = qms002.Id, StepTemplateId = stDocSect.Id, Sequence = 1,
                NameOverride = "Purpose and Scope",
                DescriptionOverride = "States the purpose of the Quality Policy and its mandatory content per ISO 9001:2015 clause 5.2."
            };
            ps1.Contents.Add(Blk(ps1, 0,
                "1.1  Purpose\n\n" +
                "This document records the organisation's Quality Policy — a formal statement by top management " +
                "of its commitment to quality and continual improvement. The Quality Policy is a mandatory piece " +
                "of documented information required by ISO 9001:2015 clause 5.2.2. It provides the framework " +
                "within which quality objectives are set and against which the effectiveness of the QMS is assessed."));
            ps1.Contents.Add(Blk(ps1, 1,
                "1.2  Mandatory content (clause 5.2.1)\n\n" +
                "Per ISO 9001:2015 clause 5.2.1, the Quality Policy shall:\n\n" +
                "  a)  Be appropriate to the purpose and context of the organisation and support its strategic direction\n" +
                ...
                "All four elements must be present and verifiable in the policy text. An auditor will check each " +
                "element explicitly."));
            ps1.Contents.Add(Blk(ps1, 2,
                "1.3  Quality Policy Statement\n\n" +
                "Our organisation is committed to delivering products and services that consistently meet or " +
                "exceed the requirements of our customers and all applicable statutory and regulatory obligations. " +
                "We achieve this through the rigorous design, implementation, and continual improvement of our " +
                "Quality Management System, operated in accordance with ISO 9001:2015.\n\n" +
                "We set measurable quality objectives aligned to our strategic direction, monitor progress against " +
                "them, and review both the objectives and this policy at each management review to ensure they " +
                "remain relevant and effective.\n\n" +
                "Every person in our organisation is responsible for the quality of their own work. We invest in " +
                "training, process clarity, and a culture of transparency so that everyone understands how their " +
                "contribution supports the quality of our outputs and the satisfaction of our customers.\n\n" +
                "                                            Signed: ________________________\n" +
                "                                            Title:  Managing Director\n" +
                "                                            Date:   ________________________"));
            qms002.ProcessSteps.Add(ps1);

            var ps2 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms002.CreatedAt, UpdatedAt = qms002.CreatedAt,
                ProcessId = qms002.Id, StepTemplateId = stDocSect.Id, Sequence = 2,
                NameOverride = "Responsibilities",
                DescriptionOverride = "Accountability for authoring, approving, communicating, and maintaining the Quality Policy."
            };
            ps2.Contents.Add(Blk(ps2, 0,
                "Top Management\n\n" +
                "Solely accountable for:\n" +
                "  • Authoring and signing the Quality Policy\n" +
                "  • Ensuring it reflects the organisation's current strategic direction and context\n" +
                "  • Approving every revision before it is released\n" +
                "  • Demonstrating commitment by acting in accordance with the policy and expecting others to do the same\n\n" +
                "The policy must be signed by the highest level of management with organisational authority — " +
                "typically the Managing Director or Chief Executive. Delegation to the Quality Manager for signature " +
                "is not acceptable; the policy must reflect personal leadership commitment."));
            ps2.Contents.Add(Blk(ps2, 1,
                "Quality Manager\n\n" +
                "Responsible for:\n" +
                "  • Facilitating the annual review of the policy with top management\n" +
                "  • Drafting proposed revisions for top-management consideration\n" +
                "  • Submitting approved revisions through the document control workflow (QMS-006)\n" +
                "  • Maintaining distribution records and evidence of awareness\n" +
                "  • Posting the current signed policy in all relevant locations (physical noticeboards, intranet, Document Library)\n" +
                "  • Including a policy review item on every management review agenda (QMS-019)"));
            ps2.Contents.Add(Blk(ps2, 2,
                "All Managers and Supervisors\n\n" +
                "Responsible for:\n" +
                "  • Communicating the policy to their teams in a way that is meaningful and relevant to day-to-day work\n" +
                "  • Including quality policy awareness in team inductions\n" +
                "  • Ensuring their team members have had the opportunity to read and ask questions about the policy\n" +
                "  • Modelling behaviours consistent with the policy's stated commitments"));
            ps2.Contents.Add(Blk(ps2, 3,
                "All Personnel\n\n" +
                "Required to:\n" +
                "  • Be aware of the Quality Policy, understand its relevance to their work, and comply with it\n" +
                "  • Acknowledge receipt and understanding of the policy at induction and on each revision\n" +
                "  • Raise concerns or suggestions for improvement to their manager or the Quality Manager\n\n" +
                "Awareness is evidenced through signed acknowledgement records, which are retained as part of the " +
                "training and competency records for each individual."));
            qms002.ProcessSteps.Add(ps2);

            var ps3 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms002.CreatedAt, UpdatedAt = qms002.CreatedAt,
                ProcessId = qms002.Id, StepTemplateId = stDocSect.Id, Sequence = 3,
                NameOverride = "Procedure",
                DescriptionOverride = "Steps for establishing, reviewing, revising, and communicating the Quality Policy."
            };
            ps3.Contents.Add(Blk(ps3, 0,
                "Step 1 — Establish the initial policy\n\n" +
                "Top management drafts a policy that satisfies all four requirements of clause 5.2.1 (see §1.2). " +
                "Before finalising, verify:\n" +
                "  • The policy reflects the current organisational context and strategic direction (informed by clause 4.1)\n" +
                "  • The policy commits to satisfying applicable requirements (customer, statutory, and regulatory)\n" +
                "  • The policy commits explicitly to continual improvement of the QMS\n" +
                "  • The policy provides a framework — not just a statement — for setting objectives (i.e. it gives " +
                "direction on what areas objectives should address)\n\n" +
                "Once satisfied, top management signs and dates the policy. The signed document is submitted " +
                "for release via the document approval workflow."));
            ps3.Contents.Add(Blk(ps3, 1,
                "Step 2 — Release and communicate\n\n" +
                "Once released, the Quality Manager ensures the policy is:\n" +
                "  • Published to the Document Library (Revision B or current version, Released status)\n" +
                "  • Posted physically at all relevant sites and work areas\n" +
                "  • Published on the organisation's intranet or equivalent internal communication channel\n" +
                "  • Included in induction training for all new employees (see TRN-SYS-001)\n" +
                "  • Available to interested parties — customers, regulatory bodies, certification auditors — on request\n\n" +
                "All employees must be given the opportunity to read the policy and ask questions. " +
                "Acknowledgement records must be obtained from all personnel within 30 days of a new release."));
            ps3.Contents.Add(Blk(ps3, 2,
                "Step 3 — Annual review\n\n" +
                "The Quality Manager adds a Quality Policy review item to every management review agenda (QMS-019). " +
                "At the review, top management considers:\n" +
                "  • Whether the policy remains appropriate to the organisation's current context and strategy\n" +
                "  • Whether the four mandatory elements remain present and meaningful\n" +
                "  • Whether the quality objectives framework the policy provides is still generating useful objectives\n" +
                "  • Feedback from audits, customer satisfaction data, and operational performance\n\n" +
                "If no change is required, the review outcome is minuted and the policy is carried forward. " +
                "If a change is required, proceed to step 4."));
            ps3.Contents.Add(Blk(ps3, 3,
                "Step 4 — Revise and re-release\n\n" +
                "Top management agrees the revised wording. The Quality Manager creates a new Draft revision in " +
                "the Document Library (incrementing the revision code), records the change description, and " +
                "submits for approval via the standard document approval workflow (see QMS-006). On release:\n" +
                "  • The previous revision is automatically marked Superseded\n" +
                "  • The effective date of the new revision is recorded\n" +
                "  • Steps 2 and 3 of the communication plan are repeated for the new revision\n" +
                "  • All personnel must re-acknowledge the revised policy; acknowledgement records are retained"));
            qms002.ProcessSteps.Add(ps3);

            var ps4 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms002.CreatedAt, UpdatedAt = qms002.CreatedAt,
                ProcessId = qms002.Id, StepTemplateId = stDocSect.Id, Sequence = 4,
                NameOverride = "Records and Documented Information",
                DescriptionOverride = "Documented information required by or associated with the Quality Policy."
            };
            ps4.Contents.Add(Blk(ps4, 0,
                "The following documented information shall be maintained or retained:\n\n" +
                "  • QMS-002 (this document) — the current signed Quality Policy; maintained as a Released controlled document in the Document Library\n" +
                "  • Top-management signature — each released revision must carry the wet or electronic signature of the authorised signatory\n" +
                "  • Distribution and posting records — evidence that the policy has been communicated (intranet timestamp, physical posting log)\n" +
                "  • Personnel acknowledgement records — signed or electronically confirmed records for each employee; " +
                "linked to each individual's training and competency file\n" +
                "  • Management review minutes — must include explicit confirmation that the Quality Policy has been reviewed; see QMS-019\n" +
                "  • Version history — all superseded revisions are retained in the Document Library version history and must not be destroyed"));
            ps4.Contents.Add(Blk(ps4, 1,
                "Retention periods\n\n" +
                "The current signed Quality Policy and all superseded revisions shall be retained for the lifetime " +
                "of the organisation's ISO 9001 certification plus a minimum of three years after any certification " +
                "lapses. Personnel acknowledgement records shall be retained for the duration of each individual's " +
                "employment plus three years. Electronic records in Process Manager are subject to the system " +
                "data retention and backup policy."));
            qms002.ProcessSteps.Add(ps4);
        }

        // ── QMS-003 — full rich content blocks (ISO 9001:2015 clause 6.2) ────
        {
            static ProcessStepContent Blk(ProcessStep ps, int order, string body) => new()
            {
                Id = Guid.NewGuid(), CreatedAt = ps.CreatedAt, UpdatedAt = ps.CreatedAt,
            ...
            var ps1 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms003.CreatedAt, UpdatedAt = qms003.CreatedAt,
                ProcessId = qms003.Id, StepTemplateId = stDocSect.Id, Sequence = 1,
                NameOverride = "Purpose and Scope",
                DescriptionOverride = "Establishes quality objectives and planning requirements per ISO 9001:2015 clause 6.2."
            };
            ps1.Contents.Add(Blk(ps1, 0,
                "1.1  Purpose\n\n" +
                "This document defines the organisation's quality objectives and specifies how achievement of " +
                "those objectives is planned, monitored, and reviewed. Quality objectives are a mandatory " +
                "requirement of ISO 9001:2015 clause 6.2. They translate the commitments in the Quality Policy " +
                "(QMS-002) into measurable targets that drive operational performance and continual improvement."));
            ps1.Contents.Add(Blk(ps1, 1,
                "1.2  Mandatory requirements (clause 6.2.1)\n\n" +
                "Quality objectives shall:\n\n" +
                "  a)  Be consistent with the Quality Policy\n" +
                "  b)  Be measurable\n" +
                "  c)  Take into account applicable requirements\n" +
                "  d)  Be relevant to conformity of products and services and customer satisfaction\n" +
                "  e)  Be monitored\n" +
                "  f)  Be communicated\n" +
                "  g)  Be updated as appropriate\n\n" +
                "For each objective, the organisation shall determine: what will be done; what resources are " +
                "required; who will be responsible; when it will be completed; and how results will be evaluated."));
            ps1.Contents.Add(Blk(ps1, 2,
                "1.3  Current quality objectives register\n\n" +
                "The live objectives register is maintained as a controlled record in the Process Manager system. " +
                "Objectives are grouped by theme and reviewed at each management review. The following table " +
                "lists the current active objectives:\n\n" +
                "Objective 1 — On-Time Delivery\n" +
                "  Measure: % of customer orders delivered on or before the confirmed delivery date\n" +
                "  Target:  ≥ 95% per rolling quarter\n" +
                "  Owner:   Operations Manager\n" +
                "  Review:  Monthly\n\n" +
                "Objective 2 — First-Pass Yield\n" +
                "  Measure: % of units passing final inspection without rework on first attempt\n" +
                "  Target:  ≥ 98% per product line per month\n" +
                "  Owner:   Quality Manager\n" +
                "  Review:  Monthly\n\n" +
                "Objective 3 — Corrective Action Closure Rate\n" +
                "  Measure: % of open corrective actions closed within agreed due date\n" +
                "  Target:  ≥ 90% on-time closure per quarter\n" +
                "  Owner:   Quality Manager\n" +
                "  Review:  Quarterly at management review\n\n" +
                "Objective 4 — Customer Satisfaction\n" +
                "  Measure: Customer satisfaction survey score (0–10 scale)\n" +
                "  Target:  Mean score ≥ 8.0 on annual survey\n" +
                "  Owner:   Commercial Director\n" +
                "  Review:  Annually, with interim review of complaints data quarterly\n\n" +
                "Objective 5 — Training Compliance\n" +
                "  Measure: % of personnel with all mandatory competencies Current (not Expired)\n" +
                "  Target:  100% at any point in time; no Expired competencies on active production roles\n" +
                "  Owner:   HR Manager\n" +
                "  Review:  Monthly via Competency Matrix"));
            qms003.ProcessSteps.Add(ps1);

            var ps2 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms003.CreatedAt, UpdatedAt = qms003.CreatedAt,
                ProcessId = qms003.Id, StepTemplateId = stDocSect.Id, Sequence = 2,
                NameOverride = "Responsibilities",
                DescriptionOverride = "Accountability for setting, monitoring, and reporting quality objectives."
            };
            ps2.Contents.Add(Blk(ps2, 0,
                "Quality Manager\n\n" +
                "  • Maintains the quality objectives register and ensures each objective meets the clause 6.2.1 criteria\n" +
                "  • Compiles monthly performance data and circulates the objectives dashboard to process owners and senior management\n" +
                "  • Prepares the objectives performance summary for each management review (QMS-019)\n" +
                "  • Initiates revisions to objectives when strategic direction changes, deviations are persistent, or new requirements emerge\n" +
                "  • Archives achieved or closed objectives with evidence of closure"));
            ps2.Contents.Add(Blk(ps2, 1,
                "Process Owners\n\n" +
                "  • Own the objectives assigned to their function and are accountable for achieving the targets\n" +
                "  • Report progress monthly to the Quality Manager, including any deviations and the corrective actions planned\n" +
                "  • Identify when an objective is no longer achievable within its current parameters and propose revisions\n" +
                "  • Ensure their team members understand the objectives relevant to their work"));
            ps2.Contents.Add(Blk(ps2, 2,
                "Top Management\n\n" +
                "  • Reviews objectives achievement at each management review and approves any additions, revisions, or closures\n" +
                "  • Approves the annual objectives plan\n" +
                "  • Allocates resources where objectives are at risk of not being met\n" +
                "  • Ensures objectives remain aligned to the organisation's strategic direction"));
            qms003.ProcessSteps.Add(ps2);

            var ps3 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms003.CreatedAt, UpdatedAt = qms003.CreatedAt,
                ProcessId = qms003.Id, StepTemplateId = stDocSect.Id, Sequence = 3,
                NameOverride = "Procedure",
                DescriptionOverride = "Steps for establishing, monitoring, reviewing, and closing quality objectives."
            };
            ps3.Contents.Add(Blk(ps3, 0,
                "Step 1 — Set or review objectives\n\n" +
                "At each management review (or whenever a significant strategic change occurs), top management " +
                "and the Quality Manager review the existing objectives and determine whether new objectives " +
                "are needed. For each objective, populate the register fields: measure, target, owner, " +
                "required resources, milestone dates, and review frequency. Verify that each objective meets " +
                "all seven criteria in clause 6.2.1 before adding it to the register."));
            ps3.Contents.Add(Blk(ps3, 1,
                "Step 2 — Communicate objectives\n\n" +
                "Once approved, the Quality Manager communicates relevant objectives to the responsible process " +
                "owners and their teams. Each objective is communicated in terms of what the team needs to do " +
                "differently or maintain, not just the abstract target figure. Objectives are published in the " +
                "quality dashboard and are available to all staff through the Document Library."));
            ps3.Contents.Add(Blk(ps3, 2,
                "Step 3 — Monitor and report\n\n" +
                "Process owners collect and report performance data against each objective at the frequency " +
                "specified in the register. The Quality Manager compiles all data into a monthly objectives " +
                "dashboard. For any objective performing below target:\n" +
                "  • Record the deviation and its likely cause\n" +
                "  • Agree and assign a corrective action with a completion date\n" +
                "  • Escalate to senior management if the deviation persists for two or more consecutive reporting periods"));
            ps3.Contents.Add(Blk(ps3, 3,
                "Step 4 — Management review\n\n" +
                "The objectives performance summary is a mandatory input to every management review (QMS-019). " +
                "At the review, management decides:\n" +
                "  • Whether each objective remains appropriate and achievable\n" +
                "  • Whether targets should be raised, maintained, or revised\n" +
                "  • Whether any objectives should be closed and replaced with new priorities\n" +
                "  • Whether resource allocation changes are needed to protect at-risk objectives\n\n" +
                "All decisions are recorded in the management review minutes with owners and due dates."));
            ps3.Contents.Add(Blk(ps3, 4,
                "Step 5 — Close and archive achieved objectives\n\n" +
                "When an objective has been sustainably met (target achieved for a minimum of three consecutive " +
                "reporting periods), the Quality Manager may propose closure to top management. On approval, " +
                "the objective is marked as Closed in the register with the closure date, final performance " +
                "data, and a brief summary of how the target was achieved. Closed objectives are retained in " +
                "the register archive as evidence of continual improvement."));
            qms003.ProcessSteps.Add(ps3);

            var ps4 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms003.CreatedAt, UpdatedAt = qms003.CreatedAt,
                ProcessId = qms003.Id, StepTemplateId = stDocSect.Id, Sequence = 4,
                NameOverride = "Records and Documented Information",
                DescriptionOverride = "Documented information required by or associated with quality objectives."
            };
            ps4.Contents.Add(Blk(ps4, 0,
                "The following documented information shall be maintained or retained:\n\n" +
                "  • Quality objectives register — the live list of current objectives with all clause 6.2.2 fields populated; " +
                "maintained by the Quality Manager; updated continuously\n" +
                "  • Monthly performance reports — data against each objective for each reporting period; retained for minimum three years\n" +
                "  • Deviation and corrective action records — where an objective is performing below target; linked to the corrective action register (QMS-020)\n" +
                "  • Management review minutes — must include objectives performance review and any decisions on additions, revisions, or closures; see QMS-019\n" +
                "  • Objective closure records — evidence of sustained achievement for each closed objective\n" +
                "  • Annual objectives plan — the agreed set of objectives for the coming year, approved by top management"));
            ps4.Contents.Add(Blk(ps4, 1,
                "Retention periods\n\n" +
                "The objectives register and all associated performance records shall be retained for a " +
                "minimum of three years from the date the objective was closed or superseded. Records " +
                "supporting management review are subject to the retention requirement of QMS-019. " +
                "Electronic records in Process Manager are subject to the system data retention and backup policy."));
            qms003.ProcessSteps.Add(ps4);
        }

        // ── QMS-004 — full rich content blocks (ISO 9001:2015 clause 4) ─────
        {
            static ProcessStepContent Blk(ProcessStep ps, int order, string body) => new()
            {
                Id = Guid.NewGuid(), CreatedAt = ps.CreatedAt, UpdatedAt = ps.CreatedAt,
            ...
            var ps1 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms004.CreatedAt, UpdatedAt = qms004.CreatedAt,
                ProcessId = qms004.Id, StepTemplateId = stDocSect.Id, Sequence = 1,
                NameOverride = "Purpose and Scope",
                DescriptionOverride = "High-level overview of the QMS structure and process interactions per ISO 9001:2015 clause 4."
            };
            ps1.Contents.Add(Blk(ps1, 0,
                "1.1  Purpose\n\n" +
                "This Quality Manual provides a high-level description of the organisation's Quality Management " +
                "System (QMS) — its structure, the interaction between its processes, and references to the " +
                "applicable procedures that govern each operational area. It is a navigational document: it " +
                "orients auditors, new employees, and customers in the overall architecture of the QMS without " +
                "duplicating the procedural detail contained in the individual procedure documents."));
            ps1.Contents.Add(Blk(ps1, 1,
                "1.2  Organisation and context (clause 4.1 and 4.2)\n\n" +
                "The organisation manufactures precision-engineered components and assemblies and provides " +
                "associated technical services to customers in the industrial, defence, and electronics sectors. " +
                "The QMS is designed to ensure consistent delivery of products and services that meet customer, " +
                "statutory, and regulatory requirements.\n\n" +
                "Key internal issues affecting the QMS include: workforce competency and retention, capacity " +
                "and equipment reliability, process maturity, and the rate of new product introduction. " +
                "Key external issues include: customer delivery and quality expectations, regulatory compliance " +
                "requirements, supply-chain performance and availability, and economic conditions affecting " +
                "material costs.\n\n" +
                "Interested parties and their relevant requirements are documented in the Interested Parties " +
                "Register maintained by the Quality Manager (referenced in QMS-001)."));
            ps1.Contents.Add(Blk(ps1, 2,
                "1.3  Scope of the QMS\n\n" +
                "The scope of the QMS is defined in QMS-001 (Quality Management System Scope). Reference that " +
                "document for the definitive scope statement, applicable products and services, sites, and any " +
                "exclusions with justification."));
            ps1.Contents.Add(Blk(ps1, 3,
                "1.4  QMS process architecture\n\n" +
                "The QMS is structured around five interacting process groups:\n\n" +
                "  1.  Leadership and planning — strategic direction, quality policy, objectives, risk management\n" +
                "      Governed by: QMS-001, QMS-002, QMS-003, QMS-005\n\n" +
                "  2.  Support — resource management, competence, infrastructure, document control, communication\n" +
                "      Governed by: QMS-006, QMS-007, QMS-008, QMS-009\n\n" +
                "  3.  Operations — customer requirements review, design and development, procurement, production, " +
                "delivery, and post-delivery activities\n" +
                "      Governed by: QMS-010, QMS-011, QMS-012, QMS-013, QMS-014, QMS-015\n\n" +
                "  4.  Quality control — nonconformance control, customer satisfaction monitoring\n" +
                "      Governed by: QMS-016, QMS-017\n\n" +
                "  5.  Performance evaluation and improvement — internal audit, management review, corrective action\n" +
                "      Governed by: QMS-018, QMS-019, QMS-020\n\n" +
                "Supporting knowledge management is addressed by QMS-021. The full document register is " +
                "maintained in the Document Library within Process Manager."));
            qms004.ProcessSteps.Add(ps1);

            var ps2 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms004.CreatedAt, UpdatedAt = qms004.CreatedAt,
                ProcessId = qms004.Id, StepTemplateId = stDocSect.Id, Sequence = 2,
                NameOverride = "Responsibilities",
                DescriptionOverride = "Accountability for maintaining and communicating the Quality Manual."
            };
            ps2.Contents.Add(Blk(ps2, 0,
                "Top Management\n\n" +
                "  • Approves each revision of the Quality Manual before release\n" +
                "  • Ensures the manual accurately reflects the current QMS design and the organisation's commitment to ISO 9001\n" +
                "  • Makes the manual available to customers and interested parties on request"));
            ps2.Contents.Add(Blk(ps2, 1,
                "Quality Manager\n\n" +
                "  • Authors and maintains the Quality Manual; proposes revisions whenever the QMS structure changes\n" +
                "  • Reviews the manual at minimum annually to confirm accuracy and completeness\n" +
                "  • Ensures all procedure references are current and the document register is up to date\n" +
                "  • Distributes the current revision to all relevant internal functions and makes it available externally"));
            ps2.Contents.Add(Blk(ps2, 2,
                "All Process Owners\n\n" +
                "  • Notify the Quality Manager when their process or procedure is significantly revised, so that the manual " +
                "can be updated to reflect the change\n" +
                "  • Confirm accuracy of the sections describing their process area during each review cycle"));
            qms004.ProcessSteps.Add(ps2);

            var ps3 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms004.CreatedAt, UpdatedAt = qms004.CreatedAt,
                ProcessId = qms004.Id, StepTemplateId = stDocSect.Id, Sequence = 3,
                NameOverride = "Procedure",
                DescriptionOverride = "Steps for maintaining, revising, and distributing the Quality Manual."
            };
            ps3.Contents.Add(Blk(ps3, 0,
                "Step 1 — Annual review\n\n" +
                "The Quality Manager reviews the Quality Manual at minimum once per year, checking:\n" +
                "  • That the organisational context description remains accurate\n" +
                "  • That the QMS process architecture section correctly maps all current procedure codes\n" +
                "  • That the scope statement cross-reference (QMS-001) is still valid\n" +
                "  • That leadership commitments and policy references are consistent with current signed versions of QMS-002 and QMS-003\n" +
                "  • That no new regulatory or normative requirements have been introduced that require coverage in the manual\n\n" +
                "The review outcome is recorded in the management review minutes (QMS-019)."));
            ps3.Contents.Add(Blk(ps3, 1,
                "Step 2 — Trigger revision\n\n" +
                "A revision is triggered by any of the following events (in addition to the annual review):\n" +
                "  • A new procedure is added to the QMS or an existing procedure is retired\n" +
                "  • The QMS scope changes (triggering a corresponding revision of QMS-001)\n" +
                "  • The organisation's context, structure, or strategic direction changes materially\n" +
                "  • A certification audit raises a finding relating to the manual's accuracy\n" +
                "  • The Quality Policy (QMS-002) is revised, requiring cross-reference update"));
            ps3.Contents.Add(Blk(ps3, 2,
                "Step 3 — Author, approve, and release\n\n" +
                "The Quality Manager creates a new Draft revision in the Document Library (incrementing the " +
                "revision code), records a meaningful change description, and submits for approval via the " +
                "standard document approval workflow (QMS-006). On release:\n" +
                "  • The previous revision is automatically marked Superseded\n" +
                "  • The current revision is communicated to all relevant parties\n" +
                "  • The distribution log is updated\n" +
                "  • The manual is made available externally where required by customer or regulatory obligations"));
            qms004.ProcessSteps.Add(ps3);

            var ps4 = new ProcessStep
            {
                Id = Guid.NewGuid(), CreatedAt = qms004.CreatedAt, UpdatedAt = qms004.CreatedAt,
                ProcessId = qms004.Id, StepTemplateId = stDocSect.Id, Sequence = 4,
                NameOverride = "Records and Documented Information",
                DescriptionOverride = "Documented information required by or associated with the Quality Manual."
            };
            ps4.Contents.Add(Blk(ps4, 0,
                "The following documented information shall be maintained or retained:\n\n" +
                "  • QMS-004 (this document) — the current Quality Manual; maintained as a Released controlled document\n" +
                "  • Version history — all superseded revisions retained in the Document Library version history\n" +
                "  • Distribution and acknowledgement log — records of who has received or been notified of each revision\n" +
                "  • Management review minutes — must include confirmation of manual review; see QMS-019\n" +
                "  • External distribution records — where the manual has been provided to customers, auditors, or regulators"));
            ps4.Contents.Add(Blk(ps4, 1,
                "Retention periods\n\n" +
                "All revisions of the Quality Manual shall be retained for the lifetime of the organisation's " +
                "ISO 9001 certification plus a minimum of three years. The manual and its version history must " +
                "be available to certification auditors on request at any time during the certification period."));
            qms004.ProcessSteps.Add(ps4);
        }
