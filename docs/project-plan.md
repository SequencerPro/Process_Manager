...
- **Phase 11 — Production Management** — expected durations + job due dates on Process steps and Jobs; equipment catalog (Equipment entity with type, location, and status); downtime event logging against equipment; preventive maintenance scheduling (PmSchedule entity, due-date calculation); production visibility dashboard (WIP board, late-job heatmap, bottleneck flags, equipment status summary)
